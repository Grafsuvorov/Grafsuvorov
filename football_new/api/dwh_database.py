        ...
        with dwh_engine.connect() as conn:
            ...
            conn.execute(text("""
                ...
                BEGIN
                    ...
                    IF NOT EXISTS (
                        SELECT 1 FROM information_schema.columns
                        WHERE table_schema = 'football' AND table_name = 'users' AND column_name = 'refresh_token'
                    ) THEN
                        ALTER TABLE football.users ADD COLUMN refresh_token VARCHAR(255);
                    END IF;

                    IF NOT EXISTS (
                        SELECT 1 FROM information_schema.columns
                        WHERE table_schema = 'football' AND table_name = 'users' AND column_name = 'refresh_token_expires'
                    ) THEN
                        ALTER TABLE football.users ADD COLUMN refresh_token_expires TIMESTAMP;
                    END IF;
                    
                    IF NOT EXISTS (
                        SELECT 1 FROM information_schema.columns
                        WHERE table_schema = 'football' AND table_name = 'users' AND column_name = 'balance'
                    ) THEN
                        ALTER TABLE football.users ADD COLUMN balance NUMERIC NOT NULL DEFAULT 0;
                    ELSE
                        ALTER TABLE football.users ALTER COLUMN balance SET DEFAULT 0;
                    END IF;

                    IF NOT EXISTS (
                        SELECT 1 FROM information_schema.columns
                        WHERE table_schema = 'football' AND table_name = 'users' AND column_name = 'subscription_status'
                    ) THEN
                        ALTER TABLE football.users ADD COLUMN subscription_status VARCHAR(20) NOT NULL DEFAULT 'free';
                    ELSE
                        ALTER TABLE football.users ALTER COLUMN subscription_status SET DEFAULT 'free';
                    END IF;

                    IF NOT EXISTS (
                        SELECT 1 FROM information_schema.columns
                        WHERE table_schema = 'football' AND table_name = 'users' AND column_name = 'subscription_until'
                    ) THEN
                        ALTER TABLE football.users ADD COLUMN subscription_until TIMESTAMP;
                    END IF;
                END $$;
            """))
            conn.commit()
...
def create_dwh_favorites_table():
    """Создает/мигрирует таблицу favorites в DWH."""
    try:
        with dwh_engine.connect() as conn:
            conn.execute(text("""
                CREATE TABLE IF NOT EXISTS football.user_favorites (
            ...
            """))
            conn.commit()
            logger.info("✅ DWH 'user_favorites' table ready")
    except Exception as e:
        logger.error(f"❌ DWH favorites table init error: {e}")
        raise
...
def create_user_in_dwh(user_data: dict):
    try:
        with dwh_engine.connect() as conn:
            balance_exists = conn.execute(text("""
                SELECT 1
            ...
            if balance_exists:
                res = conn.execute(text("""
                    INSERT INTO football.users (
                        ...
                        balance, subscription_status, created_at, updated_at
                    )
                    VALUES (
                        :email, :username, :hashed_password, :is_verified,
                        :verification_token, :verification_token_expires,
                        0, 'free', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
                    )
                    RETURNING id
                """), user_data)
            else:
...
def update_user_refresh_token_in_dwh(email: str, refresh_token: str, refresh_expires):
    try:
        with dwh_engine.connect() as conn:
            conn.execute(text("""
                UPDATE football.users
        ...
        logger.error(f"❌ update_user_refresh_token_in_dwh error: {e}")
        return False
        ...
        with dwh_engine.connect() as conn:
            row = conn.execute(text("""
                ...
                WHERE refresh_token = :refresh_token
                  AND refresh_token_expires > CURRENT_TIMESTAMP
                LIMIT 1
            """), {"refresh_token": refresh_token}).fetchone()
            return _row_to_user(row) if row else None
    except Exception as e:
        logger.error(f"❌ get_user_by_refresh_token error: {e}")
        return None
