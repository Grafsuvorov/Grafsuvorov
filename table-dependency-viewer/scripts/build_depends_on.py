...
def strip_sql_comments(sql: str) -> str:
    # Remove /* ... */ blocks (including multiline) and -- line comments
...
def build_depends_on_from_sql(
    ...
    refs = extract_schema_table_refs(sql, known_schemas=known_schemas)

    target_schema_l = target_schema.lower()
    target_table_l = target_table.lower()
    refs = {
        (schema, table)
        for (schema, table) in refs
        if not (schema == target_schema_l and table.lower() == target_table_l)
    }
...
def main() -> int:
    ...
    parser.add_argument("--report-unchanged", action="store_true", help="Print list of files not changed")
    parser.add_argument("--report-skipped", action="store_true", help="Print list of files skipped (no deps)")
    ...
    for meta_path in meta_paths:
        ...
        depends_on = build_depends_on_from_sql(
            sql_path, target_schema, target_table, known_schemas=known_schemas
        )
    ...
    if args.write:
        ...
        if args.report_unchanged and unchanged:
            print("\nUnchanged files:")
            for item in unchanged:
                print(f"  - {item}")
        if args.report_skipped and skipped:
            print("\nSkipped (no deps) files:")
            for item in skipped:
                print(f"  - {item}")
    return 0
