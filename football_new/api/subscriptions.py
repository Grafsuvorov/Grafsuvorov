...
def get_or_create_main_user(db: Session, dwh_user) -> User:
    """Сопоставляем DWH-пользователя с основной БД подписок."""
    user = db.query(User).filter(User.email == dwh_user.email).first()
    if user:
        return user
    # Если в основной БД нет записи — создаём.
    username = getattr(dwh_user, "username", None) or dwh_user.email.split("@")[0]
    is_verified = bool(getattr(dwh_user, "is_verified", True))
    user = User(
        email=dwh_user.email,
        username=username,
...
def get_current_user(
    dwh_user=Depends(get_current_user_dwh),
    db: Session = Depends(get_db),
) -> User:
    return get_or_create_main_user(db, dwh_user)
...
@router.get("/user-roi")
async def get_user_roi(
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    # Пока нет отдельной логики — отдаём заглушку без ошибки.
    return {"roi": 0.0, "period_days": 30}
