...
def get_incident(table_fqn: str = Query(..., description="Format: schema.table")):
    ...
    # 🔧 FIX: normalize_fqn теперь возвращает (schema, table)
    try:
        schema, table = normalize_fqn(table_fqn)
    except Exception:
        return JSONResponse(
            status_code=400,
            content={"error": "table_fqn must be in format schema.table"}
        )
    ...
    with engine.connect() as conn:
        ...
        if not table_id:
            return JSONResponse(
                status_code=404,
                content={"error": "table not found in tables_meta", "table_fqn": f"{schema}.{table}"}
            )
        ...
        for r in rows:
            ...
            timeline.append({
                ...
                "duration_sec": float(r["duration_seconds"]) if r["duration_seconds"] else None,
        ...
        # ---------- dependencies ----------
        try:
            deps = get_dependencies(table=f"{schema}.{table}")
        except Exception:
            deps = []
