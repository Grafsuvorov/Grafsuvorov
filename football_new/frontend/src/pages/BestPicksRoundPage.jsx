...
const API_ROUTES = {
  matches: "http://localhost:8001/api/matches_v3",
  upcoming: "http://localhost:8001/api/matches_v3",
  bestPicks: "http://localhost:8001/api/best-picks",
  insightsBatch: "http://localhost:8001/api/fixture-insights",
  insightSingle: "http://localhost:8001/api/match-insight",
};
  ...
  return (
    <button
      ...
      className="flex items-center justify-center rounded-full
                 ...
                 bg-surface-3/90 ring-1 ring-slate-700/70
                 shadow-[0_0_18px_rgba(129,140,248,0.55)]
                 hover:ring-violet-400/90 hover:shadow-[0_0_26px_rgba(196,181,253,0.7)]
                 transition"
...
function CompactPicksTable(props) {
  ...
  return (
    <div className="overflow-x-auto rounded-3xl border border-glass bg-surface-2/90">
...
function DualCard(props) {
    ...
    return (
      <div className="px-1 py-1 space-y-1">
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs sm:text-[13px] font-medium text-slate-50">
          ...
          <Pill color="indigo">{rec.label}</Pill>
          <span className="text-slate-300">
            p {fmtPct(rec.p, 0)}
          </span>
          <span className="text-slate-300">
            odds {fmtNum(rec.odds, 2)}
          </span>
          {!isNil(rec.ev) ? (
            <EVPill ev={rec.ev} />
          ) : (
            <Pill color="gray">EV —</Pill>
          )}
          <Pill color="gray">
            Kelly {fmtPct(kFull, 1)} → {fmtPct(kRec, 1)}
          </Pill>
          {rec._mode === "model" ? (
            <Pill color="blue" className="ml-1">
              по модели
            </Pill>
          ) : null}
          {decisionBadge ? (
            <Pill color="amber" className="shrink-0">
              {decisionBadge}
            </Pill>
          ) : null}
          <span className="flex-1" />
          {isHot ? (
            <Pill color="rose" className="shrink-0">
              🔥 Hot pick
            </Pill>
          ) : null}
        </div>
        ...
        {decisionBadge ? (
          <div className="mt-1 text-[11px] text-amber-300/90">
            {decisionProfile && decisionProfile !== "Baseline"
              ? decisionProfile
              : decisionBadge}
          </div>
        ) : null}
  ...
  return (
    <div className="rounded-3xl bg-gradient-to-b from-surface-2/95 via-surface-2 to-surface-3/95 border border-glass shadow-[0_0_40px_rgba(76,29,149,0.55)] hover:border-violet-500/80 hover:shadow-[0_0_64px_rgba(129,140,248,0.85)] transition overflow-hidden">
      <div className="p-4 sm:p-5">
        ...
        {(function () {
          const evOutcome = isNum(outcome?.ev) ? outcome.ev : null;
          const evTotal = isNum(total?.ev) ? total.ev : null;

          let best = null;

          if (evOutcome !== null || evTotal !== null) {
            if (evOutcome === null) best = total;
            else if (evTotal === null) best = outcome;
            else best = evOutcome >= evTotal ? outcome : total;
          }

          const shouldSkip =
            (!isNum(best?.ev) || best.ev <= 0) &&
            (!isNum(evOutcome) || evOutcome <= 0) &&
            (!isNum(evTotal) || evTotal <= 0);

          if (shouldSkip) {
            return (
              <div className="mt-4 mb-3 rounded-2xl bg-surface-3/70 border border-slate-800/60 px-3.5 py-3 shadow-[0_0_20px_rgba(15,23,42,0.7)]">
                <div className="text-sm font-semibold text-amber-300 flex items-center gap-2">
                  🤝 Матч рекомендуется пропустить
                </div>
                <div className="mt-1 text-[13px] text-slate-300/90">
          ...
          return (
            <div className="mt-4 mb-3 rounded-2xl bg-gradient-to-r from-violet-700/20 via-fuchsia-600/20 to-violet-700/20 border border-violet-500/40 px-3.5 py-3 shadow-[0_0_28px_rgba(139,92,246,0.45)]">
              <div className="flex items-center gap-2 text-sm font-semibold text-violet-200">
                🔥 Лучший выбор модели:
                <span className="text-slate-50">{best.label}</span>
              </div>
        ...
        {/* содержимое */}
        <div className="mt-3 rounded-2xl bg-surface-2/40 border border-slate-700/40 backdrop-blur-sm overflow-hidden">
...
export default function BestPicksRoundPage() {
  ...
  useEffect(
    function () {
      if (fastMode || !USE_INSIGHTS) return;
      var need = [];
      var limit = Math.min(
        visibleCount,
        selectedFixtures.length
    ...
    [selectedFixtures, visibleCount, fastMode, insightsMap]
  );

  /* ================== Рендер ================== */
  return (
    <div className="min-h-screen bg-surface-1 text-slate-50">
      <div className="mx-auto max-w-7xl px-3 sm:px-4 lg:px-6 pt-4 pb-12">
        {/* шапка страницы (без дублирующего LeagueTabsHeader) */}
        <div className="mb-5 flex items-start justify-between gap-4 flex-wrap">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full border border-glass bg-surface-2/80 px-3 py-1 text-[10px] uppercase tracking-[0.18em] text-slate-400">
              <span className="h-1.5 w-1.5 rounded-full bg-fuchsia-400 shadow-[0_0_12px_rgba(244,114,182,0.8)]" />
              EdgeScore Premium
            </div>
            <h1 className="mt-3 text-xl sm:text-2xl font-semibold text-slate-50 flex items-center gap-2">
              Подборки ставок ·{" "}
            ...
            <p className="mt-1 text-sm text-slate-400 max-w-2xl">
              Две рекомендации по каждому матчу топ-лиг: исход и
              тотал 2.5 с пояснениями модели. Страница доступна
              только подписчикам EdgeScore.
            </p>
          </div>
          <div className="flex flex-col items-end text-right text-[10px] text-slate-500">
            ...
            <span className="mt-0.5 text-xs text-slate-300">
              {BUILD_TAG}
            </span>
          </div>
        </div>
        ...
        {/* фильтры */}
        <div className="panel bg-surface-2/80 border border-glass rounded-3xl px-4 py-4 mb-5 grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
          <div className="flex flex-col gap-1">
            ...
            <input
              value={FIXED_SEASON}
              className="rounded-2xl border border-slate-700/70 bg-surface-3/80 px-3 py-2 text-sm text-slate-100 shadow-inner"
              disabled
            />
          </div>
          <div className="flex flex-col gap-1">
            ...
            <select
              ...
              className="rounded-2xl border border-slate-700/70 bg-surface-3/80 px-3 py-2 text-sm text-slate-100"
            >
          ...
          <div className="flex flex-col gap-1">
            ...
            <select
              ...
              className="rounded-2xl border border-slate-700/70 bg-surface-3/80 px-3 py-2 text-sm text-slate-100 disabled:opacity-40"
              disabled={league === "All"}
            >
          ...
          <div className="flex flex-col gap-1">
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <div className="w-14 text-right text-sm font-mono text-slate-100">
