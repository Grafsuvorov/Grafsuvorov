  ...
  useEffect(function(){
    async function fetchAll() {
      ...
      try {
        ...
        const data1 = await fetchJSONCached("/api/matches_v3?" + qs1.toString(), TTL_MATCHES);
        ...
        const upPromises = TOP_LEAGUES.map(function(L){
          ...
          return fetchJSONCached("/api/matches_v3?" + qsU.toString(), TTL_UPCOMING)
            .then(function(arr){
        ...
        const data2 = await fetchJSONCached("/api/best-picks?" + qs2.toString(), TTL_BESTPICKS);
        ...
        if (USE_INSIGHTS && ids.length) {
          try {
            const raw = await fetchJSONCached("/api/fixture-insights?fixture_ids=" + ids.join(","), TTL_INSIGHTS);
          ...
          if (missing.length) {
            const promises = missing.map(function(id){ return fetch("/api/match-insight?fixture_id=" + id); });
