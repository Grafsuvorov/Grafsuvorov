...
def create_tables():
    ...
    # API-Football отдаёт group в standings, и он нужен для маленьких групповых таблиц.
    try:
        with engine.begin() as conn:
            ...
            conn.execute(
                text(
                    """
                    CREATE INDEX IF NOT EXISTS ix_api_football_standings_lg_season_group_rank
                    ON football.api_football_standings (league_id, season, group_name, rank)
                    """
                )
            )
    except Exception as e:
        print(f"[DB] standings schema patch skipped due to error: {e}")
