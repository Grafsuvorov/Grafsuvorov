...
create table if not exists ods."/rusal/sd2882m_ral" ( -- ключ: market, reg_perio, matkl, numvr, numlinevr
	market bpchar(1),
	reg_perio varchar(6),
	matkl varchar(9),
	numvr varchar(2),
...
comment on column ods."/rusal/sd2882m_ral".ernam is 'Автор заказа ЦК из заявки (сотрудник Департамента сбыта, который отвечает за заявку клиента) | Автор заказа ЦК из заявки (сотрудник Департамента сбыта, который отвечает за заявку клиента) | stg."/RUSAL/SD2882M"."ERNAM"';
comment on column ods."/rusal/sd2882m_ral".zterm is 'Метод оплаты (код) |Метод оплаты (код) | stg."/RUSAL/SD2882M"."zterm"';
comment on column ods."/rusal/sd2882m_ral".paym_cond is 'Документ от которого идет расчет условия оплаты (код) | Документ от которого идет расчет условия оплаты (код) | stg."/RUSAL/SD2882M"."paym_cond"';
comment on column ods."/rusal/sd2882m_ral".deliv_region is 'Трейдеры: регион поставки | Трейдеры: регион поставки | stg."/RUSAL/SD2882M"."DELIV_REGION"';
