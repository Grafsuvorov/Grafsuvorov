import json
from copy import deepcopy

import pandas as pd

from config import UNDERSTAT_MIN_SEASON
from data.build_dataset import build_dataset
from data.loader import load_stats
from features.elo import build_elo_features
from features.form_xg import build_form_xg_features
from features.team_stats_form import build_team_stats_form
from features.h2h import build_h2h_features
from features.h2h_recent import build_h2h_recent_features
from features.league import build_league_context_features
from features.build_matrix import build_feature_matrix
from features.draw_diff import add_draw_diff_features
from train_outcomes import _train_outcomes_single
from train_totals import _train_totals_single


UNDERSTAT_PREFIXES = ("home_us_", "away_us_")

COMPACT_UNDERSTAT_COLUMNS = [
    "home_us_xg_all_5",
    "away_us_xg_all_5",
    "home_us_xga_all_5",
    "away_us_xga_all_5",
    "home_us_npxg_all_5",
    "away_us_npxg_all_5",
    "home_us_npxga_all_5",
    "away_us_npxga_all_5",
    "home_us_xg_all_10",
    "away_us_xg_all_10",
    "home_us_xga_all_10",
    "away_us_xga_all_10",
    "home_us_npxg_all_10",
    "away_us_npxg_all_10",
    "home_us_npxga_all_10",
    "away_us_npxga_all_10",
    "home_us_deep_all_5",
    "away_us_deep_all_5",
    "home_us_deep_allowed_all_5",
    "away_us_deep_allowed_all_5",
    "home_us_ppda_all_5",
    "away_us_ppda_all_5",
    "home_us_ppda_allowed_all_5",
    "away_us_ppda_allowed_all_5",
]


def _weighted_metric(metrics_by_league, metric_name):
    num = 0.0
    den = 0
    for metrics in metrics_by_league.values():
        n = int(metrics.get("val_n", 0))
        val = metrics.get(metric_name)
        if n <= 0 or val is None:
            continue
        num += float(val) * n
        den += n
    return (num / den) if den else None


def _add_compact_understat_derived(df_all: pd.DataFrame) -> pd.DataFrame:
    df = df_all.copy()
    specs = {
        "us_attack_edge_5": ("home_us_xg_all_5", "away_us_xga_all_5"),
        "us_attack_edge_10": ("home_us_xg_all_10", "away_us_xga_all_10"),
        "us_def_edge_5": ("away_us_xg_all_5", "home_us_xga_all_5"),
        "us_def_edge_10": ("away_us_xg_all_10", "home_us_xga_all_10"),
        "us_npxg_edge_5": ("home_us_npxg_all_5", "away_us_npxga_all_5"),
        "us_npxg_edge_10": ("home_us_npxg_all_10", "away_us_npxga_all_10"),
        "us_home_control_5": ("home_us_deep_all_5", "away_us_deep_allowed_all_5"),
        "us_away_control_5": ("away_us_deep_all_5", "home_us_deep_allowed_all_5"),
    }
    for out_col, (left_col, right_col) in specs.items():
        if left_col in df.columns and right_col in df.columns:
            df[out_col] = pd.to_numeric(df[left_col], errors="coerce") - pd.to_numeric(df[right_col], errors="coerce")
    return df


def _build_feature_frame(include_understat: bool, compact_understat: bool = False) -> pd.DataFrame:
    df_all = build_dataset(return_all=True)
    df_all = df_all[df_all["season"].astype(int) >= int(UNDERSTAT_MIN_SEASON)].copy()

    match_stats = load_stats()
    if not match_stats.empty:
        match_stats = match_stats[match_stats["fixture_id"].isin(df_all["fixture_id"])].copy()

    feats_list = [
        build_elo_features(df_all, mode="train"),
        build_form_xg_features(df_all, match_stats, window=5),
        build_team_stats_form(df_all, match_stats, window=5),
        build_h2h_features(df_all, mode="train"),
        build_h2h_recent_features(df_all, window=5),
        build_league_context_features(df_all, window=60),
    ]
    df_all = build_feature_matrix(schedule=df_all, feats_list=feats_list, target_df=None)
    df_all = add_draw_diff_features(df_all)

    understat_cols = [c for c in df_all.columns if c.startswith(UNDERSTAT_PREFIXES)]
    if not include_understat:
        drop_cols = list(understat_cols)
        df_all = df_all.drop(columns=drop_cols, errors="ignore")
    elif compact_understat:
        df_all = _add_compact_understat_derived(df_all)
        keep = set(COMPACT_UNDERSTAT_COLUMNS)
        keep.update(c for c in df_all.columns if c.startswith("us_"))
        drop_cols = [c for c in understat_cols if c not in keep]
        df_all = df_all.drop(columns=drop_cols, errors="ignore")

    return df_all[df_all["has_result"]].copy()


def _train_by_league(df_all: pd.DataFrame):
    outcome_metrics = {}
    totals_metrics = {}

    league_ids = sorted({int(x) for x in df_all["league_id"].dropna().unique()})
    for lid in league_ids:
        subset = df_all[df_all["league_id"] == lid].copy()
        if subset.empty:
            continue
        outcome_metrics[lid] = _train_outcomes_single(subset, league_id=lid)["metrics"]
        totals_metrics[lid] = _train_totals_single(subset)["metrics"]

    return outcome_metrics, totals_metrics


def _summarize(metrics_by_league):
    return {
        "weighted_val_ll": _weighted_metric(metrics_by_league, "val_ll"),
        "weighted_val_acc": _weighted_metric(metrics_by_league, "val_acc"),
        "weighted_val_brier": _weighted_metric(metrics_by_league, "val_brier"),
        "val_n_total": int(sum(int(m.get("val_n", 0)) for m in metrics_by_league.values())),
        "by_league": deepcopy(metrics_by_league),
    }


def main():
    baseline_df = _build_feature_frame(include_understat=False)
    understat_df = _build_feature_frame(include_understat=True)
    compact_understat_df = _build_feature_frame(include_understat=True, compact_understat=True)

    base_out, base_tot = _train_by_league(baseline_df)
    us_out, us_tot = _train_by_league(understat_df)
    compact_out, compact_tot = _train_by_league(compact_understat_df)

    report = {
        "window": {
            "season_from": int(UNDERSTAT_MIN_SEASON),
            "rows_baseline": int(len(baseline_df)),
            "rows_understat": int(len(understat_df)),
        },
        "baseline": {
            "outcomes": _summarize(base_out),
            "totals": _summarize(base_tot),
        },
        "with_understat": {
            "outcomes": _summarize(us_out),
            "totals": _summarize(us_tot),
        },
        "with_understat_compact": {
            "outcomes": _summarize(compact_out),
            "totals": _summarize(compact_tot),
        },
    }

    base_out_ll = report["baseline"]["outcomes"]["weighted_val_ll"]
    us_out_ll = report["with_understat"]["outcomes"]["weighted_val_ll"]
    base_tot_ll = report["baseline"]["totals"]["weighted_val_ll"]
    us_tot_ll = report["with_understat"]["totals"]["weighted_val_ll"]

    report["delta"] = {
        "outcomes_val_ll": None if base_out_ll is None or us_out_ll is None else us_out_ll - base_out_ll,
        "totals_val_ll": None if base_tot_ll is None or us_tot_ll is None else us_tot_ll - base_tot_ll,
        "outcomes_val_acc": (
            None
            if report["baseline"]["outcomes"]["weighted_val_acc"] is None
            or report["with_understat"]["outcomes"]["weighted_val_acc"] is None
            else report["with_understat"]["outcomes"]["weighted_val_acc"]
            - report["baseline"]["outcomes"]["weighted_val_acc"]
        ),
        "totals_val_acc": (
            None
            if report["baseline"]["totals"]["weighted_val_acc"] is None
            or report["with_understat"]["totals"]["weighted_val_acc"] is None
            else report["with_understat"]["totals"]["weighted_val_acc"]
            - report["baseline"]["totals"]["weighted_val_acc"]
        ),
        "compact_outcomes_val_ll": (
            None
            if base_out_ll is None or report["with_understat_compact"]["outcomes"]["weighted_val_ll"] is None
            else report["with_understat_compact"]["outcomes"]["weighted_val_ll"] - base_out_ll
        ),
        "compact_totals_val_ll": (
            None
            if base_tot_ll is None or report["with_understat_compact"]["totals"]["weighted_val_ll"] is None
            else report["with_understat_compact"]["totals"]["weighted_val_ll"] - base_tot_ll
        ),
        "compact_outcomes_val_acc": (
            None
            if report["baseline"]["outcomes"]["weighted_val_acc"] is None
            or report["with_understat_compact"]["outcomes"]["weighted_val_acc"] is None
            else report["with_understat_compact"]["outcomes"]["weighted_val_acc"]
            - report["baseline"]["outcomes"]["weighted_val_acc"]
        ),
        "compact_totals_val_acc": (
            None
            if report["baseline"]["totals"]["weighted_val_acc"] is None
            or report["with_understat_compact"]["totals"]["weighted_val_acc"] is None
            else report["with_understat_compact"]["totals"]["weighted_val_acc"]
            - report["baseline"]["totals"]["weighted_val_acc"]
        ),
    }

    print(json.dumps(report, ensure_ascii=False, indent=2))


if __name__ == "__main__":
    main()
