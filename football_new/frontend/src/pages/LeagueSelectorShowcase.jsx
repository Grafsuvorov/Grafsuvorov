  ...
  return (
    <div
      className={`h-7 w-7 shrink-0 rounded-full grid place-items-center text-[10px] font-semibold border ${
        active ? "text-white border-transparent" : "text-gray-700 border-gray-300"
      }`}
      style={{
        background: active
...
function Chip({ active, color, children, onClick }) {
  return (
    <button
      onClick={onClick}
      className={`flex items-center gap-2 rounded-full px-3 py-1.5 text-sm border transition whitespace-nowrap ${
        active
        ...
        <tbody className="text-sm">
          {TOP_PLAYERS_MOCK.map((r, i) => (
            <tr key={i} className={i % 2 ? "bg-white" : "bg-gray-50/40"}>
              <td className="px-4 py-3 text-gray-500">{r.pos}</td>
              <td className="px-4 py-3">
                <div className="flex items-center gap-3">
                  <div className="h-8 w-8 rounded-full bg-gray-200" />
                  <div>
                    <div className="font-medium">{r.name}</div>
                    <div className="text-xs text-gray-500">{r.club}</div>
                  </div>
                </div>
              </td>
      ...
      >
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              className="rounded-lg bg-white px-3 py-1.5 text-sm text-gray-900 hover:bg-gray-50"
              title="Все лиги"
...
function SideRail({ active, onSelect, onOpenPalette }) {
  ...
  return (
    <div className="flex gap-4">
      <nav className="sticky top-4 h-fit w-16 rounded-2xl border border-gray-200 bg-white p-2 shadow-sm">
        <div className="flex flex-col items-center gap-2">
          {LEAGUES.map((l) => (
            ...
            >
              <Logo label={l.short} active={active === l.id} color={l.color} />
              <span className="pointer-events-none absolute left-9 top-1/2 -translate-y-1/2 whitespace-nowrap rounded-md bg-gray-900 px-2 py-1 text-xs text-white opacity-0 shadow-sm transition group-hover:opacity-100">
                {l.name}
              </span>
            </button>
          ))}
...
function CommandModal({ open, onClose, onSelect }) {
  ...
  return (
    <div className="fixed inset-0 z-50">
      ...
      <div className="absolute left-1/2 top-24 w-[min(760px,92vw)] -translate-x-1/2 rounded-2xl bg-white shadow-2xl">
...
export default function LeagueSelectorShowcase() {
  ...
  return (
    <div className="min-h-[100vh] bg-gray-50 p-5">
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-3">
              {grouped[country].map((l) => (
                <button
                  key={l.id}
                  onClick={() => onSelect(l.id)}
                  className={`flex items-center gap-3 rounded-xl border p-3 text-left shadow-sm transition hover:shadow ${
                    active === l.id ? "border-black" : "border-gray-200"
                  }`}
                >
                  <Logo
                    label={l.short}
