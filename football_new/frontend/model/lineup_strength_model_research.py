...
LINEUP_PREFIXES = ("home_ls_", "away_ls_", "ls_")
...
def _drop_lineup_cols(df: pd.DataFrame) -> pd.DataFrame:
    drop_cols = [c for c in df.columns if c.startswith(LINEUP_PREFIXES)]
...
def main():
    ...
    baseline_df = _drop_lineup_cols(full_df)

    base_out, base_tot = _train_by_league(baseline_df)
    ls_out, ls_tot = _train_by_league(full_df)

    lineup_cols = sorted(c for c in full_df.columns if c.startswith(LINEUP_PREFIXES))

    report = {
        "window": {
            "season_from": int(UNDERSTAT_MIN_SEASON),
            "rows": int(len(full_df)),
            "lineup_feature_count": int(len(lineup_cols)),
        },
        "baseline": {
            "outcomes": _summarize(base_out),
            "totals": _summarize(base_tot),
        },
        "with_lineup_strength": {
            "outcomes": _summarize(ls_out),
            "totals": _summarize(ls_tot),
        },
        "lineup_feature_columns": lineup_cols,
    }

    report["delta"] = {
        "outcomes_val_ll": (
            report["with_lineup_strength"]["outcomes"]["weighted_val_ll"]
            - report["baseline"]["outcomes"]["weighted_val_ll"]
        ),
        "outcomes_val_acc": (
            report["with_lineup_strength"]["outcomes"]["weighted_val_acc"]
            - report["baseline"]["outcomes"]["weighted_val_acc"]
        ),
        "totals_val_ll": (
            report["with_lineup_strength"]["totals"]["weighted_val_ll"]
            - report["baseline"]["totals"]["weighted_val_ll"]
        ),
        "totals_val_acc": (
            report["with_lineup_strength"]["totals"]["weighted_val_acc"]
            - report["baseline"]["totals"]["weighted_val_acc"]
        ),
        "totals_val_brier": (
            report["with_lineup_strength"]["totals"]["weighted_val_brier"]
