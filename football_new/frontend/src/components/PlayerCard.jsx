...
export default function PlayerCard({ visible, player, meta, side, onClose, isMVP }) {
    ...
    (async () => {
      try {
        const res = await fetch(
          `/api/player/overview?player_id=${pid}`
        );
        const data = await res.json();
        if (!cancel) setExtra(data || null);
      } catch {
        if (!cancel) setExtra(null);
      }
    })();
    return () => {
      cancel = true;
  ...
  const position =
    ...
    getPlayerPosition(meta) ||
    getPlayerPosition(extra) ||
    null;
  const age =
    getPlayerAge(player) ||
    getPlayerAge(meta) ||
  ...
  const Section = ({ title, rows }) => (
    <div className="rounded-xl border border-glass bg-surface-2 shadow-sm">
      <div className="px-3 py-2 text-xs font-semibold text-slate-200 border-b border-glass">{title}</div>
      <div className="p-3 grid grid-cols-2 gap-x-4 gap-y-2 text-sm text-slate-100">
        {rows.map((r, i) => (
    ...
    <div className="fixed inset-0 z-[100]">
      ...
      <div
        className="
          ...
          w-[min(700px,92vw)]
          max-h-[70vh]
          rounded-2xl bg-surface-1/95 shadow-2xl border border-glass text-slate-100
          grid grid-rows-[auto,1fr] overflow-hidden
        "
      >
        {/* sticky header */}
        <div className={`sticky top-0 z-10 bg-surface-1/95 backdrop-blur border-b border-glass px-4 transition-all ${compact ? "py-2" : "py-3"}`}>
          <div className="flex items-center gap-3">
            ...
            <div className="ml-auto text-right">
              <div className="text-[11px] text-slate-400">Рейтинг</div>
            ...
            <button
              className="ml-2 text-slate-400 hover:text-white text-xl leading-none"
              aria-label="close"
              onClick={onClose}
              title="Закрыть"
            >
