drop table if exists pg_temp.sdt18;
create temp table pg_temp.sdt18 as (
--with sdt18 as (
	select
		lddr.delivery_code
...
drop table if exists pg_temp.vbln;
create temp table pg_temp.vbln as (
--,vbln as ( --cte1 из t5
	select -- distinct 
		...
		,ssms.tsw_location_code													-- Направление (код) SD.000008
	...
	from
		...
		left join pg_temp.sdt18	as sdt18_3
	  		...
	  		and rn = 1
	  	left join pg_temp.sdt18	as sdt18_4
...
drop table if exists pg_temp.fwpd;
create temp table pg_temp.fwpd as (
	select
		transport_hub_code
		,is_terminal_code
		...
		,market_region1_code
	from
		dict_dds.foreign_warehouse_priority_definition
...
drop table if exists pg_temp.t5;
create temp table pg_temp.t5 as (
--,t5 as 
	select --distinct
		...
		,t4.dt_created_r
		,t4.transport_route_departure_hub_code									-- Начальный транспортный узел на данном этапе перевозки (код)
		...
		,vbln.tsw_location_code													-- Направление (код) SD.000008
		,vbln.tsw_location_name 												-- Направление SD.000009
	...
	from
		...
		left join pg_temp.vbln 
			on t4.sales_bundle_code = vbln.sales_bundle_code
			and rn = 1
		left join pg_temp.fwpd as fwpd
...
drop table if exists pg_temp.vbss_vbsk;
create temp table pg_temp.vbss_vbsk as (
--,vbss_vbsk as (
...
drop table if exists pg_temp.uzb;
create temp table pg_temp.uzb as (
--,uzb as (
	select
		uzb.ident
		,uzb.num_mh1
		...
		,ls.location_name
	from
		ods.ztsd5018m_uz_b_ral as uzb 
		join dict_dds.location_sales as ls
			on ls.location_code = uzb.locid
)
distributed by (ident);

drop table if exists pg_temp.eub;
create temp table pg_temp.eub as (
--,eub as (
	select
		eub.ident
		...
		,ls.transport_hub_code
	from
		ods.ztsd5018m_eu_b_ral as eub 
		join dict_dds.location_sales as ls
			on ls.location_code = eub.locid
)
distributed by (ident);

drop table if exists pg_temp.vbsk_y;
create temp table pg_temp.vbsk_y as (
--,vbsk_y as (
	select 
		bol.dt_discharge
...
drop table if exists pg_temp.outb;
create temp table pg_temp.outb as (
--,outb as (
	select 
		ls.transport_hub_code 
		,outb.vbeln 
		...
		,outb.charg 
	from
		ods.ztsd5018m_outb_b_ral as outb
...
drop table if exists pg_temp.lgort;
create temp table pg_temp.lgort as (
--,lgort as (
	select
		sales_bundle_code
		...
		,case 
			when dt_transportation_stage_start_r is not null
				then transportation_outbound_stage_code
			else transportation_inbound_stage_code
		end as current_transportation_stage_code
		,knote
		,dt_transportation_stage_start_p
		,dt_transportation_stage_start_r		
		,case 
			when is_foreign_warehouse_code is not null
				then row_number() over(
					partition by sales_bundle_code, is_foreign_warehouse_code is not null 
					order by dt_transportation_stage_start_p asc, dt_transportation_stage_start_r asc)
		end as rn
		,row_number() over (
			partition by sales_bundle_code
			order by dt_transportation_stage_start_p desc, dt_transportation_stage_start_r desc
		) as rn_dt_desc
	from pg_temp.t5 as t5
--	where 
--		is_foreign_warehouse_code is not null
)
distributed by (sales_bundle_code);

drop table if exists pg_temp.vbsk_o;
create temp table pg_temp.vbsk_o as (
--,vbsk_o as (
	select
		vbss.delivery_code
...
drop table if exists pg_temp.s_zrepo;
create temp table pg_temp.s_zrepo as (
--,s_zrepo as (
	select distinct
		range_low_value 											
	from
	...
	where
		abap_program_code = '/RUSAL/SD2973M_3' 
...
drop table if exists pg_temp.stat1;
create temp table pg_temp.stat1 as (
	--,stat1 as (
		select
			scm_pledge_status_name 
		from
			dict_dds.scm_pledge_status_texts
		where
			scm_pledge_status_code = '1'
		limit 1
	)
distributed replicated;

drop table if exists pg_temp.nomtk_s;
create temp table pg_temp.nomtk_s as (
	select
		range_low_value 											
	from
		dict_dds.settings_and_parameters_sap 
	where
		abap_program_code = '/RUSAL/SD3275M' 
...
drop table if exists pg_temp.dds__sales_document_counterparty_role;
create temp table pg_temp.dds__sales_document_counterparty_role as (
	select 
		sales_document_code, 
		supplier_code , 
		counterparty_role_code
	from dds.sales_document_counterparty_role
	where counterparty_role_code in ('XR', 'ZU')
)
distributed by (sales_document_code);

drop table if exists pg_temp.result_01;
create temp table pg_temp.result_01 as (
select
	...
	,t5.tsw_location_code																													-- Направление (код) SD.000008
	...
	,t5.transport_route_departure_hub_code																									-- Начальный транспортный узел на данном этапе перевозки (код)
	,lgort_desc.current_transportation_stage_code																							-- SD.001398 Последний этап (код)
	
	,uzb1.num_mh1 as uzb1__num_mh1, uzb1.num_mh3 as uzb1__num_mh3, uzb1.notification as uzb1__notification, uzb1.sammg_n as uzb1__sammg_n, uzb1.traid_out as uzb1__traid_out
	,eub1.sammg_n as eub1__sammg_n, eub1.notification as eub1__notification,eub1.container_out as eub1__container_out, eub1.capacity as eub1__capacity
	,sd4798_35.dt_transportation_stage_start as sd4798_35__dt_transportation_stage_start
	,eub2.notification as eub2__notification, eub2.sammg_n as eub2__sammg_n
	,t5.port_of_discharge_in_foreign_port_code
	,t5.bill_of_lading_group_code
	,t5.lot_customer_code
	,t5.market_region1_code
	,t5.is_terminal_code ,t5.is_foreign_warehouse_code,t5.is_temporary_warehouse_code
	,t5.external_contract_in_lot_number
	,sd4798_w03.delivery_code as sd4798_w03__delivery_code
	,sd4798_w04.delivery_code as sd4798_w04__delivery_code
	,sd4798_w03_2.delivery_code as sd4798_w03_2__delivery_code
	,sd4798_w04_2.delivery_code as sd4798_w04_2__delivery_code
from pg_temp.t5 as t5
	...
	left join pg_temp.lgort as lgort_min
		on lgort_min.sales_bundle_code = t5.sales_bundle_code
		and lgort_min.rn = 1
	left join pg_temp.lgort as lgort_max
		on lgort_max.sales_bundle_code = t5.sales_bundle_code
		and lgort_max.rn = 2
	left join pg_temp.lgort as lgort_desc
		on lgort_desc.sales_bundle_code = t5.sales_bundle_code
		and lgort_desc.rn_dt_desc = 1		
...
drop table if exists pg_temp.result_02; 
create temp table pg_temp.result_02 as (
select
	t5.delivery_number_initial																-- Исходная поставка SD.000001					--2025.05.22 ДОБАВИТЬ: поле уже есть
	,t5.port_of_discharge_code																-- Порт выгрузки (код) SD.000044
	,t5.port_of_discharge_name 																-- Порт выгрузки SD.000045						--2025.05.22 ДОБАВИТЬ: поле уже есть
 	,t5.bill_of_lading_in_foreign_port														-- Коносамент в ин.порту SD.000048				--2025.05.22 ДОБАВИТЬ: поле уже есть
 	,t5.dt_storage_start									-- Дата начала хранения ин. склад SD.000418
 	,t5.dt_storage_end									-- Дата окончания хранения ин. склад SD.000419
 	,t5.warehouse_code																-- Удаленный склад (код) SD.000420
 	,t5.sales_bundle_code																	-- ID химии SD.000511
 	,vv.dt_end_of_free_storage as dt_storage_end_in_release									-- Дата окончания хранения на складе за счет RUSAL SD.000546
 	,t5.sales_delivery_code										-- Поставка SD.000548
 	,lips.plant_producer_code as receiving_plant_code										-- Принимающий завод SD.000549
 	,t5.sales_bundle_gross_weight 						-- Вес брутто пакета SD.000722
	,t5.sales_bundle_net_weight 							-- Вес нетто пакета SD.000723 
	,t5.sales_bundle_net_weight_with_wirerod -- Вес Н&К пакета SD.000724
 	,t5.batch_code																			-- Партия SD.000737
	,t5.uni																					-- UNI SD.000151							--2025.05.28 добавила
	,CASE
		WHEN lips.plant_producer_code IS NOT NULL THEN
			(CASE
	...
	,t5.dt_pb3_number																		-- Дата PB 3 SD.000753							--2025.07.23 добавила	
	,CASE
		WHEN t5.dt_storage_end IS NOT NULL
			AND vv.dt_end_of_free_storage IS NOT NULL
		THEN
			CASE
				WHEN t5.dt_storage_end < vv.dt_end_of_free_storage
					OR vv.dt_end_of_free_storage < t5.dt_storage_start
				THEN t5.dt_storage_end
				WHEN vv.dt_end_of_free_storage >= t5.dt_storage_start
				THEN vv.dt_end_of_free_storage
				ELSE NULL
			END
        ELSE
			CASE
				WHEN t5.dt_storage_end IS NOT NULL
				THEN t5.dt_storage_end
				WHEN vv.dt_end_of_free_storage IS NOT NULL
					AND vv.dt_end_of_free_storage >= t5.dt_storage_start
				THEN vv.dt_end_of_free_storage
				ELSE NULL
			END
    ...
    ,t5.delivery_code_le_p																													-- LE поставка прихода SD.000513				
    ,t5.delivery_code_le_r																													-- LE поставка расхода SD.000516				
    ,t5.barcode_ean_code 																												-- Штриховой код SD.000929						
    ,coalesce(t5.uzb1__num_mh1, ddh3.mh1_storage_document_number) as fwrd_info_mh1_storage_document_number										-- EXP: № Акта МХ-1	SD.000930		
	,coalesce(t5.uzb1__num_mh3, ddh3.mh3_storage_document_number) as fwrd_info_mh3_storage_document_number										-- EXP: № Акта МХ-3	SD.000931							
	,coalesce(t5.sd4798_35__dt_transportation_stage_start , vbsk_y.dt_discharge) as dt_fwrd_info_discharge_in_foreign_port						-- EXP: Дата выгрузки в порту SD.000932		
	,t5.dt_fwrd_info_storage_start_in_foreign_port												-- EXP: Начало хранения ин. склад 1 SD.000933
    ,t5.dt_fwrd_info_storage_end_in_foreign_port													-- EXP: Окончание хранение ин. склад 1 SD.000934         
	,coalesce(t5.eub1__notification, t5.uzb1__notification, sdt18_1.shipment_instruction_number) as fwrd_info_shipment_instruction_number 			-- EXP: Инструкция на отгрузку Ин Порт SD.000935		
	,coalesce(t5.eub1__sammg_n, t5.uzb1__sammg_n, sdt18_1.shipment_instruction_code) as fwrd_info_shipment_instruction_code						-- EXP: Группа инструкции на отгрузку Ин Порт SD.000936 
	,t5.fwrd_info_delivery_notice_number																						-- EXP: Номер нотиса о доставке	SD.000938		
	,coalesce(t5.eub1__container_out, t5.uzb1__traid_out, ddh3.vehicle_in_foreign_port_code) as fwrd_info_transport_vehicle_in_foreign_port_code 	-- EXP: Номер ТС в ин. Порту SD.000939 	
	,coalesce(t5.eub1__capacity, outb1.capacity) as fwrd_info_transport_capacity_amount														-- EXP: Грузоподъемность SD.000940				
	,t5.fwrd_info_second_foreign_warehouse_location_name										-- EXP: Storage location 2 SD.000941 
	,t5.dt_fwrd_info_storage_start_in_second_foreign_warehouse									-- EXP: Начало хранения ин. склад 2 SD.000942
	,t5.dt_fwrd_info_storage_end_in_second_foreign_warehouse										-- EXP: Окончание хранение ин. склад 2 SD.000943
	,coalesce(t5.eub2__notification, outb2.notification, sdt18_2.shipment_instruction_number) as fwrd_info_shipment_instruction_in_second_foreign_port_number	-- EXP: Инструкция на отгрузку Ин Порт 2 SD.000944   УДАЛИТЬ! 
	,coalesce(t5.eub2__notification, outb2.notification, sdt18_2.shipment_instruction_number) as fwrd_info_shipment_instruction_in_2nd_foreign_port_number		-- EXP: Инструкция на отгрузку Ин Порт 2 SD.000944
	,coalesce(t5.eub2__sammg_n, outb2.sammg_n, sdt18_2.shipment_instruction_code) as fwrd_info_shipment_instruction_in_second_foreign_port_code-- EXP: Группа инструкции на отгрузку Ин Порт 2 SD.000945	УДАЛИТЬ! 	
	,coalesce(t5.eub2__sammg_n, outb2.sammg_n, sdt18_2.shipment_instruction_code) as fwrd_info_shipment_instruction_in_2nd_foreign_port_code	-- EXP: Группа инструкции на отгрузку Ин Порт 2 SD.000945	 
	,t5.transportation_stage35_delivery_code																		-- LE поставка Этап 35 SD.000953	  			
	,t5.transportation_stage40_delivery_code																		-- LE поставка Этап 40 SD.000954			   
    ,t5.transportation_stage42_delivery_code																		-- LE поставка Этап 42 SD.000955			    
    ,t5.transportation_stage55_delivery_code																		-- LE поставка Этап 55 SD.000956	  			
    ,t5.transportation_stage60_delivery_code																		-- LE поставка Этап 60 SD.000957 	  				   
	,t5.transportation_stage_final_delivery_code																		-- LE поставка последней операции SD.000958
    ,t5.sales_order_in_shipment 																											-- Заказ ЦК в отгрузке SD.000005
	,t5.dt_final_release 																													-- Дата Финальный релиз SD.000482
	,t5.final_release_code 																													-- Номер Финальный релиз SD.000588
	,t5.dt_shipment_instruction_in_foreign_port																								-- Дата инструкции на отгрузку Ин Порт SD.000613
	,t5.dt_shipment_instruction_in_second_foreign_port																						-- Дата инструкции на отгрузку Ин Порт 2 SD.000619
	,t5.final_release_internal_code																											-- Группа Финальный релиз SD.000952  
    ,t5.forwarder_departure_transportation_stage_code /*transportation_stage_code*/							-- Этап расхода (код) SD.001043			УДАЛИТЬ!
	,t5.transportation_stage_code /*2025-10-02 добавляю*/														-- Этап перевозки SD.000529				УДАЛИТЬ!
	,t5.transportation_inbound_stage_code																									-- Код этапа прихода SD.000529
	,t5.transportation_outbound_stage_code																									-- Код этапа расхода SD.001043
    ,t5.dt_transportation_delivery_created																					-- Дата создания LE-поставки расхода SD.001044    
  	,t5.delivery_basis 																														-- Базис поставки SD.000067
	,t5.delivery_point_name 																												-- Пункт доставки по инкотермс SD.000068 
	,t5.pb_number 																															-- LotWshe/PB number SD.000158
	,t5.russian_port_bill_of_lading_forwarder_code 																							-- EXP: WH Operator's code SD.000632  -- НА УДАЛЕНИЕ ПОСЛЕ ЗАМЕНЫ В SS
	...
	,t5.uzbekistan_cargo_declaration_73																										-- EXP: ГТД ИМ73 SD.000634
	,t5.port_of_loading_name																												-- Порт погрузки SD.000653
	,t5.location_type_of_remote_warehouse_name																							-- Признак (Ин.склад/СВХ/Терминал/Порт РФ) SD.001218
	,t5.transportation_delivery_to_foreign_1st_warehouse_code																			-- LE поставка хранения SD.000241
	,t5.transportation_delivery_to_foreign_2nd_warehouse_code																			-- LE поставка хранения 2 SD.001278
	,t5.transportation_stage66_delivery_code																		-- LE поставка Этап 66 SD.001273
	,t5.transportation_stage77_delivery_code																		-- LE поставка Этап 77 SD.001274
	,t5.transportation_stage49_delivery_code																		-- LE поставка Этап 49 SD.001275
	,t5.delivery_number_of_producer_plant																									-- Номер поставки завода производителя SD.000003
	,t5.tsw_location_code																													-- Направление (код) SD.000008
	,t5.tsw_location_name 																													-- Направление SD.000009
	,t5.dt_arrival_by_railway																												-- Дата прибытия по ЖД SD.000011
	...
	,t5.port_of_loading_code 																												-- Порт погрузки (код) SD.000649
	,t5.transport_route_departure_hub_code																									-- Начальный транспортный узел на данном этапе перевозки (код)
	,t5.current_transportation_stage_code																							-- SD.001398 Последний этап (код)
	
	,vbsk_o.pledge_reserve_code as vbsk_o__pledge_reserve_code
	,t5.port_of_discharge_in_foreign_port_code
	,t5.bill_of_lading_group_code
	,t5.lot_customer_code
	,t5.market_region1_code
	,t5.is_terminal_code ,t5.is_foreign_warehouse_code,t5.is_temporary_warehouse_code
	,t5.external_contract_in_lot_number
	,t5.sd4798_w03__delivery_code, t5.sd4798_w04__delivery_code, t5.sd4798_w03_2__delivery_code, t5.sd4798_w04_2__delivery_code
	,case 
		when t5.market_region1_code = '03'
			and (t5.is_terminal_code = 'X' 				--ZSD2973M_LGORT-SIGN_TERMINAL
				or t5.is_foreign_warehouse_code = 'X'		--ZSD2973M_LGORT-SIGN_FOREIGN_WH
				or t5.is_temporary_warehouse_code = 'X')	--ZSD2973M_LGORT-SIGN_TSW
		then t5.sd4798_w03__delivery_code
		when t5.market_region1_code = '04'
			and (t5.is_terminal_code = 'X' 				--ZSD2973M_LGORT-SIGN_TERMINAL
				or t5.is_foreign_warehouse_code = 'X'		--ZSD2973M_LGORT-SIGN_FOREIGN_WH
				or t5.is_temporary_warehouse_code = 'X')	--ZSD2973M_LGORT-SIGN_TSW
		then t5.sd4798_w04__delivery_code																									
	end as sales_document_code_sdcr_1
	,case 
		when t5.market_region1_code = '03'
			and (t5.is_terminal_code = 'X' 				--ZSD2973M_LGORT-SIGN_TERMINAL
				or t5.is_foreign_warehouse_code = 'X'		--ZSD2973M_LGORT-SIGN_FOREIGN_WH
				or t5.is_temporary_warehouse_code = 'X')	--ZSD2973M_LGORT-SIGN_TSW
		then t5.sd4798_w03_2__delivery_code
		when t5.market_region1_code = '04'
			and (t5.is_terminal_code = 'X' 				--ZSD2973M_LGORT-SIGN_TERMINAL
				or t5.is_foreign_warehouse_code = 'X'		--ZSD2973M_LGORT-SIGN_FOREIGN_WH
				or t5.is_temporary_warehouse_code = 'X')	--ZSD2973M_LGORT-SIGN_TSW
		then t5.sd4798_w04_2__delivery_code																									
	end as sales_document_code_sdcr_2	
from pg_temp.result_01 as t5
	left join pg_temp.vbss_vbsk	as vv										
		on vv.delivery_code = t5.sales_delivery_code  
		and vv.rn = 1  
	left join dds.delivery_document_position as lips									
		on lips.delivery_code = t5.sales_delivery_code
		and lips.plant_producer_code is not null 
		and lips.delivery_position_line_item_code = '000010'
	left join dds.delivery_document_header as ddh3 				
		on ddh3.delivery_code = t5.sales_delivery_code		
	left join pg_temp.vbsk_y as vbsk_y											
		on vbsk_y.delivery_code = t5.sales_delivery_code	
		and vbsk_y.rn = 1
	left join pg_temp.outb as outb1												
		on outb1.vbeln = t5.sales_delivery_code
		and outb1.transport_hub_code = t5.port_of_discharge_code
		and outb1.rn = 1		
	left join pg_temp.sdt18	as sdt18_1											
		on sdt18_1.delivery_code = t5.sales_delivery_code	
		and sdt18_1.rn = 1
	left join pg_temp.outb as outb2	 											
		on outb2.vbeln = t5.sales_delivery_code
		and outb2.transport_hub_code = t5.port_of_discharge_in_foreign_port_code
		and outb2.rn = 1		
	left join pg_temp.sdt18	as sdt18_2											
		on sdt18_2.delivery_code = t5.sales_delivery_code	
		and sdt18_2.rn = 2
	left join pg_temp.vbsk_o as vbsk_o
		on vbsk_o.delivery_code = t5.sales_delivery_code	
) distributed by (delivery_code_le_r);-- (sales_bundle_code);

drop table if exists pg_temp.result_03;
create temp table pg_temp.result_03 as (
select
	t5.delivery_number_initial																-- Исходная поставка SD.000001					--2025.05.22 ДОБАВИТЬ: поле уже есть
	,t5.port_of_discharge_code																-- Порт выгрузки (код) SD.000044
	,t5.port_of_discharge_name 																-- Порт выгрузки SD.000045						--2025.05.22 ДОБАВИТЬ: поле уже есть
 	...
 	,t5.warehouse_code																-- Удаленный склад (код) SD.000420
 	,t5.sales_bundle_code																	-- ID химии SD.000511
 	,t5.dt_storage_end_in_release									-- Дата окончания хранения на складе за счет RUSAL SD.000546
 	,t5.sales_delivery_code										-- Поставка SD.000548
 	,t5.receiving_plant_code										-- Принимающий завод SD.000549
 	,t5.sales_bundle_gross_weight 						-- Вес брутто пакета SD.000722
	,t5.sales_bundle_net_weight 							-- Вес нетто пакета SD.000723 
	,t5.sales_bundle_net_weight_with_wirerod -- Вес Н&К пакета SD.000724
 	,t5.batch_code																			-- Партия SD.000737
	,t5.uni																					-- UNI SD.000151							--2025.05.28 добавила
	,t5.metal_owner_for_reporting_name													-- Собственник SD.000544						--2025.05.28 добавила
 	,t5.forwarder_name																		-- Экспедитор SD.000021							--2025.05.28 добавила
 	,t5.bill_of_lading_number																-- Номер коносамента SD.000041					--2025.05.28 добавила
	,t5.pb1_number																			-- Номер PB 1 SD.000592							--2025.07.23 добавила
	,t5.pb2_number																			-- Номер PB 2 SD.000593							--2025.07.23 добавила
	...
	,t5.dt_pb2_number																		-- Дата PB 2 SD.000752							--2025.07.23 добавила
	,t5.dt_pb3_number																		-- Дата PB 3 SD.000753							--2025.07.23 добавила	
	,t5.dt_shipment_from_foreign_warehouse -- SD.000547 | Дата ухода со склада		
    ,t5.delivery_code_le_p																													-- LE поставка прихода SD.000513				
    ,t5.delivery_code_le_r																													-- LE поставка расхода SD.000516				
    ,t5.barcode_ean_code 																												-- Штриховой код SD.000929						
    ,t5.fwrd_info_mh1_storage_document_number										-- EXP: № Акта МХ-1	SD.000930		
	,t5.fwrd_info_mh3_storage_document_number										-- EXP: № Акта МХ-3	SD.000931							
	,t5.dt_fwrd_info_discharge_in_foreign_port						-- EXP: Дата выгрузки в порту SD.000932		
	,t5.dt_fwrd_info_storage_start_in_foreign_port												-- EXP: Начало хранения ин. склад 1 SD.000933
    ,t5.dt_fwrd_info_storage_end_in_foreign_port													-- EXP: Окончание хранение ин. склад 1 SD.000934         
	,t5.fwrd_info_shipment_instruction_number 			-- EXP: Инструкция на отгрузку Ин Порт SD.000935		
	,t5.fwrd_info_shipment_instruction_code						-- EXP: Группа инструкции на отгрузку Ин Порт SD.000936 
	,ddh.transport_bill_code as fwrd_info_transport_bill_external_number																	-- EXP: Номер накладной SD.000937     			
	,t5.fwrd_info_delivery_notice_number																						-- EXP: Номер нотиса о доставке	SD.000938		
	,t5.fwrd_info_transport_vehicle_in_foreign_port_code 	-- EXP: Номер ТС в ин. Порту SD.000939 	
	,t5.fwrd_info_transport_capacity_amount														-- EXP: Грузоподъемность SD.000940				
	,t5.fwrd_info_second_foreign_warehouse_location_name										-- EXP: Storage location 2 SD.000941 
	,t5.dt_fwrd_info_storage_start_in_second_foreign_warehouse									-- EXP: Начало хранения ин. склад 2 SD.000942
	,t5.dt_fwrd_info_storage_end_in_second_foreign_warehouse										-- EXP: Окончание хранение ин. склад 2 SD.000943
	,t5.fwrd_info_shipment_instruction_in_second_foreign_port_number	-- EXP: Инструкция на отгрузку Ин Порт 2 SD.000944   УДАЛИТЬ! 
	,t5.fwrd_info_shipment_instruction_in_2nd_foreign_port_number		-- EXP: Инструкция на отгрузку Ин Порт 2 SD.000944
	,t5.fwrd_info_shipment_instruction_in_second_foreign_port_code-- EXP: Группа инструкции на отгрузку Ин Порт 2 SD.000945	УДАЛИТЬ! 	
	,t5.fwrd_info_shipment_instruction_in_2nd_foreign_port_code	-- EXP: Группа инструкции на отгрузку Ин Порт 2 SD.000945	 
	...
	,t5.bill_of_lading_created_by_name																										-- Создатель коносамента в ин. Порту SD.000951 
	,case 
		when vbsk_y_smg.nomination_code in (select range_low_value from pg_temp.nomtk_s)
	...
	,t5.shipment_instruction_in_foreign_port_name																							-- Инструкция на отгрузку Ин Порт SD.000612
	,t5.shipment_instruction_in_second_foreign_port_name																					-- Инструкция на отгрузку Ин Порт 2 SD.000618 
	,t5.dt_vehicle_loaded    																												-- EXP: Load out date SD.000629
	,t5.location_comment 																													-- EXP: Storage location SD.000630
	,t5.plant_producer_name 																												-- Завод SD.000007
	,t5.plant_owner_code 																													-- Завод собственник (код) SD.000099
	,t5.sales_order 																														-- Заказ ЦК SD.000123
	,t5.dt_delivery_notice 																													-- Дата нотиса о доставке SD.000132
	...
	,t5.lot_customer_code	
	,t5.external_contract_in_lot_number
	,sch.sales_contract_type_code as sch__sales_contract_type_code
from pg_temp.result_02 as t5
	...
	left join dds.pledge_reserve as vbsk_sh
		on vbsk_sh.pledge_reserve_code = t5.vbsk_o__pledge_reserve_code--vbsk_o.pledge_reserve_code
	...
	left join pg_temp.dds__sales_document_counterparty_role as sdcr 
		on sdcr.sales_document_code = t5.sales_document_code_sdcr_1
		and sdcr.counterparty_role_code = 'XR'
	left join pg_temp.dds__sales_document_counterparty_role as sdcr_2 
		on sdcr_2.sales_document_code = t5.sales_document_code_sdcr_2
		and sdcr_2.counterparty_role_code = 'XR'
	left join pg_temp.dds__sales_document_counterparty_role as sdcr_3 
		on sdcr_3.sales_document_code = t5.sales_document_code_sdcr_1
		and sdcr_3.counterparty_role_code = 'ZU'
	left join pg_temp.dds__sales_document_counterparty_role as sdcr_4 
		on sdcr_4.sales_document_code = t5.sales_document_code_sdcr_2
...
) distributed by (sales_delivery_code);-- (sales_bundle_code);

drop table if exists pg_temp.result_02;

drop table if exists pg_temp.result_04;
create temp table pg_temp.result_04 as (
select
	t5.delivery_number_initial																-- Исходная поставка SD.000001					--2025.05.22 ДОБАВИТЬ: поле уже есть
	,t5.port_of_discharge_code																-- Порт выгрузки (код) SD.000044
	,t5.port_of_discharge_name 																-- Порт выгрузки SD.000045						--2025.05.22 ДОБАВИТЬ: поле уже есть
	...
	,t5.sales_bundle_net_weight_with_wirerod -- Вес Н&К пакета SD.000724
 	...
 	,tvknt.location_code as location_of_remote_warehouse_code								-- Локация удаленного склада (код)
	...
	,t5.fwrd_info_shipment_instruction_number 			-- EXP: Инструкция на отгрузку Ин Порт SD.000935		
	,t5.fwrd_info_shipment_instruction_code						-- EXP: Группа инструкции на отгрузку Ин Порт SD.000936 
	,t5.fwrd_info_transport_bill_external_number																	-- EXP: Номер накладной SD.000937     			
	,t5.fwrd_info_delivery_notice_number																						-- EXP: Номер нотиса о доставке	SD.000938		
	,t5.fwrd_info_transport_vehicle_in_foreign_port_code 	-- EXP: Номер ТС в ин. Порту SD.000939 	
	,t5.fwrd_info_transport_capacity_amount														-- EXP: Грузоподъемность SD.000940				
	,t5.fwrd_info_second_foreign_warehouse_location_name										-- EXP: Storage location 2 SD.000941 
	...
	,t5.fwrd_info_shipment_instruction_in_second_foreign_port_code-- EXP: Группа инструкции на отгрузку Ин Порт 2 SD.000945	УДАЛИТЬ! 	
	,t5.fwrd_info_shipment_instruction_in_2nd_foreign_port_code	-- EXP: Группа инструкции на отгрузку Ин Порт 2 SD.000945	 
	,t5.pledge_contract_external_number																	-- Номер контракта Pledge reserve SD.000946
	,t5.final_pledge_in_bank_code --нет данных															-- Банк Pledge reserve (код) SD.000947
	,t5.final_pledge_in_bank_name																				-- Название Банка Pledge reserve SD.000948
	,case 
	 	when t5.sch__sales_contract_type_code =  (select range_low_value from pg_temp.s_zrepo) --'ZREP'---??? нет условия
	  	then (select scm_pledge_status_name from pg_temp.stat1) --'Хранение за счет RM'
	  	else 
	  		case 
	...
	,t5.dt_shipment_instruction_in_second_foreign_port																						-- Дата инструкции на отгрузку Ин Порт 2 SD.000619
	,t5.final_release_internal_code																											-- Группа Финальный релиз SD.000952  
    ,t5.vehicle_in_transportation_delivery_code																			-- Транспортное средство LE-поставки расхода SD.001042 
    ,t5.forwarder_departure_transportation_stage_code /*transportation_stage_code*/							-- Этап расхода (код) SD.001043			УДАЛИТЬ!
	,t5.transportation_stage_code /*2025-10-02 добавляю*/														-- Этап перевозки SD.000529				УДАЛИТЬ!
	,t5.transportation_inbound_stage_code																									-- Код этапа прихода SD.000529
	,t5.transportation_outbound_stage_code																									-- Код этапа расхода SD.001043
	...
	,t5.pb_number 																															-- LotWshe/PB number SD.000158
	,t5.russian_port_bill_of_lading_forwarder_code 																							-- EXP: WH Operator's code SD.000632  -- НА УДАЛЕНИЕ ПОСЛЕ ЗАМЕНЫ В SS
	,t5.forwarder_storing_in_foreign_1st_warehouse_code																	-- EXP: WH Operator's code SD.000632
	,t5.owner_plant_name 																													-- Завод собственник SD.000655
	,t5.forwarder_in_foreign_port_code																										-- Экспедитор в иностранном порту (код) SD.000950		
	,t5.bill_of_lading_created_by_name																										-- Создатель коносамента в ин. Порту SD.000951 
	,t5.railway_train_number 																											-- Номер поезда SD.000637 
	,t5.shipment_instruction_in_foreign_port_name																							-- Инструкция на отгрузку Ин Порт SD.000612
	,t5.shipment_instruction_in_second_foreign_port_name																					-- Инструкция на отгрузку Ин Порт 2 SD.000618 
	,t5.dt_vehicle_loaded    																												-- EXP: Load out date SD.000629
	,t5.location_comment 																													-- EXP: Storage location SD.000630
	...
	,t5.transportation_stage77_delivery_code																		-- LE поставка Этап 77 SD.001274
	,t5.transportation_stage49_delivery_code																		-- LE поставка Этап 49 SD.001275
	,t5.forwarder_storing_in_foreign_2nd_warehouse_code																-- EXP: WH Operator's code 2 SD.000633
	,t5.forwarder_delivering_to_foreign_1st_warehouse_code																-- EXP: Inb.carrier's code SD.001276
	,t5.forwarder_delivering_to_foreign_2nd_warehouse_code																-- EXP: Inb.carrier's code 2 SD.001279
	...
	,t5.transport_route_departure_hub_code																									-- Начальный транспортный узел на данном этапе перевозки (код)
	,t5.current_transportation_stage_code																							-- SD.001398 Последний этап (код)
from pg_temp.result_03 as t5
	left join dict_dds.transport_hub_texts as tvknt 	
		on tvknt.transport_hub_code = t5.warehouse_code  -- t5.knote уже выгружается ,t5.knote as warehouse_code																-- Удаленный склад (код) SD.000420
		and tvknt.language_code = 'E'  
	left join dict_dds.transport_hub as tvkn 				 	
		on tvkn.transport_hub_code = t5.warehouse_code
	...
	left join dict_dds.market_region1_texts as t25a1 	
		on t25a1.market_region1_code = r_sd556.market_region1_code 
		and t25a1.language_code = 'R'		
	left join dict_dds.location_sales as oijloc 		
		on oijloc.location_code = tvknt.location_code
		and tvknt.transport_hub_code = t5.warehouse_code		
	left join dict_dds.counterparty as ctp2
		on ctp2.counterparty_code = t5.forwarder_in_foreign_port_code		
	left join dict_dds.tech_zsd2921m_sc as zmsr
	--left join ods.zsd2921m_sc_ral as zmsr
		on zmsr.kunag = t5.lot_customer_code
	left join dict_dds.scm_pledge_status_texts as pst
		on pst.scm_pledge_status_code = zmsr."sign" 
		and pst.language_code = 'R'
	left join dict_dds.counterparty as ctp3
		on ctp3.counterparty_code = t5.forwarder_delivering_to_foreign_1st_warehouse_code--sdcr_3.supplier_code
		and ctp3.is_deleted is null
	left join dict_dds.counterparty as ctp4
		on ctp4.counterparty_code = t5.forwarder_delivering_to_foreign_2nd_warehouse_code--sdcr_4.supplier_code
		and ctp4.is_deleted is null  
	left join dict_dds.counterparty as ctp5
		on ctp5.counterparty_code = t5.forwarder_storing_in_foreign_1st_warehouse_code--sdcr.supplier_code
		and ctp5.is_deleted is null  
	left join dict_dds.counterparty as ctp6
		on ctp6.counterparty_code = t5.forwarder_storing_in_foreign_2nd_warehouse_code--sdcr_2.supplier_code
...
--insert into dm.sales_material_turnover_detailed_20251010 (
	...
	,location_of_remote_warehouse_code		-- Локация удаленного склада (код)
...
/*2025.07.23 новые поля из ОД*/ 	
	...
	,transport_route_departure_hub_code													-- Начальный транспортный узел на данном этапе перевозки (код)
	,current_transportation_stage_code													-- SD.001398 Последний этап (код)
)
select
	t5.delivery_number_initial																-- Исходная поставка SD.000001					--2025.05.22 ДОБАВИТЬ: поле уже есть
	,t5.port_of_discharge_code																-- Порт выгрузки (код) SD.000044
	,t5.port_of_discharge_name 																-- Порт выгрузки SD.000045						--2025.05.22 ДОБАВИТЬ: поле уже есть
 	...
 	,t5.dt_storage_end									-- Дата окончания хранения ин. склад SD.000419
 	,t5.warehouse_code																-- Удаленный склад (код) SD.000420
 	,t5.warehouse_name												-- Удаленный склад SD.000421
 	,t5.country_of_remote_warehouse_name							-- Страна удаленного склада SD.000423
 	,t5.sales_bundle_code																	-- ID химии SD.000511
 	,t5.dt_storage_end_in_release									-- Дата окончания хранения на складе за счет RUSAL SD.000546
 	,t5.sales_delivery_code										-- Поставка SD.000548
 	,t5.receiving_plant_code										-- Принимающий завод SD.000549
 	,t5.forwarder_in_foreign_port_name							-- Экспедитор в иностранном порту SD.000609	
 	,t5.sales_bundle_gross_weight 						-- Вес брутто пакета SD.000722
	,t5.sales_bundle_net_weight 							-- Вес нетто пакета SD.000723 
	,t5.sales_bundle_net_weight_with_wirerod -- Вес Н&К пакета SD.000724
 	,t5.country_of_remote_warehouse_code									-- Страна удаленного склада (код) SD.000725
 	,t5.region_of_remote_warehouse_code							-- Регион удаленного склада (код) SD.000726
 	,t5.region_of_remote_warehouse_name							-- Регион удаленного склада SD.000727 
 	,t5.location_of_remote_warehouse_code								-- Локация удаленного склада (код)
 	,t5.location_of_remote_warehouse_name								-- Локация удаленного склада SD.000728			--2025.05.22 ДОБАВИТЬ: поле уже есть
 	,t5.batch_code																			-- Партия SD.000737
	,t5.uni																					-- UNI SD.000151							--2025.05.28 добавила
	,t5.metal_owner_for_reporting_name													-- Собственник SD.000544						--2025.05.28 добавила
 	,t5.forwarder_name																		-- Экспедитор SD.000021							--2025.05.28 добавила
	...
	,t5.final_pledge_in_bank_code --нет данных															-- Банк Pledge reserve (код) SD.000947
	,t5.final_pledge_in_bank_name																				-- Название Банка Pledge reserve SD.000948
	,t5.scm_pledge_status_name																											-- Признак ЦП SD.000949	
	,t5.transportation_stage35_delivery_code																		-- LE поставка Этап 35 SD.000953	  			
	,t5.transportation_stage40_delivery_code																		-- LE поставка Этап 40 SD.000954			   
    ,t5.transportation_stage42_delivery_code																		-- LE поставка Этап 42 SD.000955			    
    ,t5.transportation_stage55_delivery_code																		-- LE поставка Этап 55 SD.000956	  			
	...
	,t5.forwarder_delivering_to_foreign_1st_warehouse_code																-- EXP: Inb.carrier's code SD.001276
	,t5.forwarder_delivering_to_foreign_2nd_warehouse_code																-- EXP: Inb.carrier's code 2 SD.001279
	,t5.forwarder_delivering_to_foreign_1st_warehouse_name														-- EXP: Inb.carrier SD.001277
	,t5.forwarder_delivering_to_foreign_2nd_warehouse_name														-- EXP: Inb.carrier 2 SD.001280
	,t5.forwarder_storing_in_foreign_1st_warehouse_name														-- EXP: WH Operator's SD.001281
	,t5.forwarder_storing_in_foreign_2nd_warehouse_name														-- EXP: WH Operator's 2 SD.001282
	,t5.delivery_number_of_producer_plant																									-- Номер поставки завода производителя SD.000003
	,t5.tsw_location_code																													-- Направление (код) SD.000008
	,t5.tsw_location_name 																													-- Направление SD.000009
	,t5.dt_arrival_by_railway																												-- Дата прибытия по ЖД SD.000011
	...
	,t5.transport_route_departure_hub_code																									-- Начальный транспортный узел на данном этапе перевозки (код)
	,t5.current_transportation_stage_code																							-- SD.001398 Последний этап (код)
from pg_temp.result_04 as t5
union all 
--Список складов
select 
	null as delivery_number_initial																/*SD.000001 | Исходная поставка*/
	,null as port_of_discharge_code																/*SD.000044 | Порт выгрузки (код)*/
	,null as port_of_discharge_name																/*SD.000045 | Порт выгрузки*/
	,null as bill_of_lading_in_foreign_port														/*SD.000048 | Коносамент в ин.порту*/
	,'1900-01-01' as dt_storage_start															/*SD.000418 | Дата начала хранения ин. склад*/
	,'2299-12-31' as dt_storage_end																/*SD.000419 | Дата окончания хранения ин. склад*/
	,fwpd.transport_hub_code as warehouse_code													/*SD.000420 | Удаленный склад (код)*/
	,tvknt.transport_hub_name as warehouse_name 												/*SD.000421 | Удаленный склад*/
	,r_sd556t.country_full_name as country_of_remote_warehouse_name								-- Страна удаленного склада SD.000423
	,null as sales_bundle_code																	/*SD.000511 | ID химии*/
	,null as dt_storage_end_in_release															/*SD.000546 | Дата окончания хранения на складе за счет RUSAL*/
	,null as sales_delivery_code																/*SD.000548 | Поставка*/
	,null as receiving_plant_code																/*SD.000549 | Принимающий завод*/
	,null as forwarder_in_foreign_port_name														/*SD.000609 | Экспедитор в иностранном порту*/
	,null as sales_bundle_gross_weight															/*SD.000722 | Вес брутто пакета | Вес брутто металла для одного пакета, в тоннах*/
	,null as sales_bundle_net_weight															/*SD.000723 | Вес нетто пакета | Вес нетто металла для одного пакета, в тоннах*/
	,null as sales_bundle_net_weight_with_wirerod												/*SD.000724 | Вес Н&К пакета | Вес нетто + катанка металла для одного пакета, в тоннах*/
	,adrc1.country_code as country_of_remote_warehouse_code										-- Страна удаленного склада (код) SD.000725
	,r_sd556.market_region1_code as region_of_remote_warehouse_code								-- Регион удаленного склада (код) SD.000726
	,t25a1.market_region1_name as region_of_remote_warehouse_name								-- Регион удаленного склада SD.000727 
	,tvknt.location_code as location_of_remote_warehouse_code									-- Локация удаленного склада (код)
	,oijloc.location_name as location_of_remote_warehouse_name									/*SD.000728 | Локация удаленного склада | Наименование локации/города, где находится склад*/
	,null as batch_code																			/*SD.000737 | Партия*/
	,null as uni																				/*SD.000151 | UNI*/
 	,null as metal_owner_for_reporting_name														/*SD.000544 | Собственник*/
	...
	,null as dt_pb1_number																		-- Дата PB 1 SD.000751				
	,null as dt_pb2_number																		-- Дата PB 2 SD.000752				
	,null as dt_pb3_number																		-- Дата PB 3 SD.000753				
	,null as dt_shipment_from_foreign_warehouse 												-- SD.000547 | Дата ухода со склада
	,null as delivery_code_le_p																	-- LE поставка прихода SD.000513				
    ,null as delivery_code_le_r																	-- LE поставка расхода SD.000516				
    ,null as barcode_ean_code 																	-- Штриховой код SD.000929						
    ,null as fwrd_info_mh1_storage_document_number												-- EXP: № Акта МХ-1	SD.000930					
    ,null as fwrd_info_mh3_storage_document_number												-- EXP: № Акта МХ-3	SD.000931						
    ,null as dt_fwrd_info_discharge_in_foreign_port												-- EXP: Дата выгрузки в порту SD.000932			
    ,null as dt_fwrd_info_storage_start_in_foreign_port											-- EXP: Начало хранения ин. склад 1 SD.000933
    ,null as dt_fwrd_info_storage_end_in_foreign_port											-- EXP: Окончание хранение ин. склад 1 SD.000934 
    ,null as fwrd_info_shipment_instruction_number 												-- EXP: Инструкция на отгрузку Ин Порт SD.000935		
	,null as fwrd_info_shipment_instruction_code												-- EXP: Группа инструкции на отгрузку Ин Порт SD.000936 
	,null as fwrd_info_transport_bill_external_number											-- EXP: Номер накладной SD.000937     			
	,null as fwrd_info_delivery_notice_number													-- EXP: Номер нотиса о доставке	SD.000938		
	,null as fwrd_info_transport_vehicle_in_foreign_port_code 									-- EXP: Номер ТС в ин. Порту SD.000939 			
	,null as fwrd_info_transport_capacity_amount												-- EXP: Грузоподъемность SD.000940
	,null as fwrd_info_second_foreign_warehouse_location_name									-- EXP: Storage location 2 SD.000941 				
	,null as dt_fwrd_info_storage_start_in_second_foreign_warehouse								-- EXP: Начало хранения ин. склад 2 SD.000942
	,null as dt_fwrd_info_storage_end_in_second_foreign_warehouse								-- EXP: Окончание хранение ин. склад 2 SD.000943
	,null as fwrd_info_shipment_instruction_in_second_foreign_port_number						-- EXP: Инструкция на отгрузку Ин Порт 2 SD.000944  УДАЛИТЬ! 
	,null as fwrd_info_shipment_instruction_in_2nd_foreign_port_number							-- EXP: Инструкция на отгрузку Ин Порт 2 SD.000944
	,null as fwrd_info_shipment_instruction_in_second_foreign_port_code							-- EXP: Группа инструкции на отгрузку Ин Порт 2 SD.000945  УДАЛИТЬ!
	,null as fwrd_info_shipment_instruction_in_2nd_foreign_port_code							-- EXP: Группа инструкции на отгрузку Ин Порт 2 SD.000945
	,null as pledge_contract_external_number													-- Номер контракта Pledge reserve SD.000946	
	,null as final_pledge_in_bank_code															-- Банк Pledge reserve (код) SD.000947
	,null as final_pledge_in_bank_name															-- Название Банка Pledge reserve SD.000948
	,null as scm_pledge_status_name																-- Признак ЦП SD.000949				
	,null as transportation_stage35_delivery_code												-- LE поставка Этап 35 SD.000953	  			
	,null as transportation_stage40_delivery_code												-- LE поставка Этап 40 SD.000954	  			
	,null as transportation_stage42_delivery_code												-- LE поставка Этап 42 SD.000955	  			
	,null as transportation_stage55_delivery_code												-- LE поставка Этап 55 SD.000956	  			
	,null as transportation_stage60_delivery_code												-- LE поставка Этап 60 SD.000957	  			
	,null as transportation_stage_final_delivery_code											-- LE поставка последней операции SD.000958
	,null as sales_order_in_shipment 															-- Заказ ЦК в отгрузке SD.000005
	...
	,null as final_release_internal_code 														-- Группа Финальный релиз SD.000952      
	,null as vehicle_in_transportation_delivery_code											-- Транспортное средство LE-поставки расхода SD.001042 
    ,null as forwarder_departure_transportation_stage_code /*transportation_stage_code*/		-- Этап расхода (код) SD.001043								УДАЛИТЬ!
	,null as transportation_stage_code /*2025-10-02 добавляю*/									-- Этап перевозки SD.000529									УДАЛИТЬ!
	,null as transportation_inbound_stage_code													-- Код этапа прихода SD.000529
	,null as transportation_outbound_stage_code													-- Код этапа расхода SD.001043
	,null as dt_transportation_delivery_created													-- Дата создания LE-поставки расхода SD.001044  
	,null as delivery_basis 																	-- Базис поставки SD.000067
	,null as delivery_point_name 																-- Пункт доставки по инкотермс SD.000068 
	,null as pb_number 																			-- LotWshe/PB number SD.000158
	,null as russian_port_bill_of_lading_forwarder_code 										-- EXP: WH Operator's code SD.000632 -- НА УДАЛЕНИЕ ПОСЛЕ ЗАМЕНЫ В SS
	,null as forwarder_storing_in_foreign_1st_warehouse_code									-- EXP: WH Operator's code SD.000632
	,null as owner_plant_name 																	-- Завод собственник SD.000655
	,null as forwarder_in_foreign_port_code														-- Экспедитор в иностранном порту (код) SD.000950
	,null as bill_of_lading_created_by_name														-- Создатель коносамента в ин. Порту SD.000951 
	,null as railway_train_number 																-- Номер поезда SD.000637
	...
	,null as dt_vehicle_loaded    																-- EXP: Load out date SD.000629
	,null as location_comment 																	-- EXP: Storage location SD.000630
	,null as plant_producer_name 																-- Завод SD.000007
	,null as plant_owner_code 																	-- Завод собственник (код) SD.000099
	,null as sales_order 																		-- Заказ ЦК SD.000123
	...
	,null as material_shape_name_full															-- Форма SD.000180
	,null as incoterms_plan_code																-- Плановый базис поставки 1 SD.000255
	,null as uzbekistan_cargo_declaration_73													-- EXP: ГТД ИМ73 SD.000634
	,null as port_of_loading_name																-- Порт погрузки SD.000653  
	,case
		when fwpd.is_foreign_warehouse_code = 'X'
			then 'Ин.склад'
		when fwpd.is_temporary_warehouse_code = 'X'
			then 'СВХ'
		when fwpd.is_terminal_code = 'X'
			then 'Терминал'
		when fwpd.is_russian_port_code = 'X'
			then 'Порт РФ'
	end as location_type_of_remote_warehouse_name												-- Признак (Ин.склад/СВХ/Терминал/Порт РФ) SD.001218
	,null as transportation_delivery_to_foreign_1st_warehouse_code								-- LE поставка хранения SD.000241
	,null as transportation_delivery_to_foreign_2nd_warehouse_code								-- LE поставка хранения 2 SD.001278
	,null as transportation_stage66_delivery_code												-- LE поставка Этап 66 SD.001273
	,null as transportation_stage77_delivery_code												-- LE поставка Этап 77 SD.001274
	,null as transportation_stage49_delivery_code												-- LE поставка Этап 49 SD.001275
	,null as forwarder_storing_in_foreign_2nd_warehouse_code									-- EXP: WH Operator's code 2 SD.000633
	,null as forwarder_delivering_to_foreign_1st_warehouse_code									-- EXP: Inb.carrier's code SD.001276
	,null as forwarder_delivering_to_foreign_2nd_warehouse_code									-- EXP: Inb.carrier's code 2 SD.001279
	,null as forwarder_delivering_to_foreign_1st_warehouse_name									-- EXP: Inb.carrier SD.001277
	,null as forwarder_delivering_to_foreign_2nd_warehouse_name									-- EXP: Inb.carrier 2 SD.001280
	,null as forwarder_storing_in_foreign_1st_warehouse_name									-- EXP: WH Operator's SD.001281
	,null as forwarder_storing_in_foreign_2nd_warehouse_name									-- EXP: WH Operator's 2 SD.001282
	,null as delivery_number_of_producer_plant													-- Номер поставки завода производителя SD.000003
	,null as tsw_location_code																	-- Направление (код) SD.000008
	,null as tsw_location_name 																	-- Направление SD.000009
	...
	,null as transport_type_after_repackaging_code												-- Тип ПС после перетарки SD.000027
	...
	,null as delivery_region_name																-- Регион поставки по контракту SD.000338
	,null as dt_shipment																		-- Дата отгрузки SD.000010
	,null as port_of_loading_code 																-- Порт погрузки (код) SD.000649
	,null as transport_route_departure_hub_code													-- Начальный транспортный узел на данном этапе перевозки (код)
	,null as current_transportation_stage_code													-- SD.001398 Последний этап (код)
from 
	pg_temp.fwpd as fwpd
	left join dict_dds.transport_hub_texts as tvknt
		on tvknt.transport_hub_code = fwpd.transport_hub_code
		and tvknt.language_code = 'E'	
	left join dict_dds.transport_hub as tvkn 				 	
		  on tvkn.transport_hub_code = fwpd.transport_hub_code
	left join dict_dds.location_sales as oijloc 		
		on oijloc.location_code = tvknt.location_code 
	left join dict_dds.address as adrc1 									 	
		on adrc1.address_code = tvkn.address_code
	...
	left join dict_dds.market_region1_texts as t25a1 	
		...
		and t25a1.language_code = 'R'	
where 
	is_foreign_warehouse_code = 'X'			--ZSD2973M_LGORT-SIGN_FOREIGN_WH
	or is_temporary_warehouse_code = 'X'	--ZSD2973M_LGORT-SIGN_TSW
...
drop table if exists pg_temp.vbln;
drop table if exists pg_temp.t5;
drop table if exists pg_temp.adrc1;
drop table if exists pg_temp.vbss_vbsk;
drop table if exists pg_temp.lips;
drop table if exists pg_temp.uzb;
drop table if exists pg_temp.eub;
drop table if exists pg_temp.vbsk_y;
drop table if exists pg_temp.outb;
...
drop table if exists pg_temp.s_zrepo;
drop table if exists pg_temp.stat1;
drop table if exists pg_temp.sd4798;
drop table if exists pg_temp.nomtk_s;
drop table if exists pg_temp.result_04;
