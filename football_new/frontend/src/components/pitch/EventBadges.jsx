...
export default function EventBadges({ meta = {}, rating, goals, assists, isMVP }) {
  ...
  const pillCls = rating != null ? ratingClasses(r, isMVP) : "bg-gray-200 text-gray-700";
    ...
    <>
      ...
      {g > 0 && (
        <div className="absolute -top-2 -left-2">
          <span className="px-1 py-[1px] text-[10px] rounded bg-white/90 shadow border">⚽{g > 1 ? `×${g}` : ""}</span>
        </div>
      )}
      {a > 0 && (
        <div className="absolute -top-2 -right-2">
          <span className="px-1 py-[1px] text-[10px] rounded bg-white/90 shadow border font-bold">
            A{a > 1 ? `×${a}` : ""}
          </span>
        </div>
      )}
