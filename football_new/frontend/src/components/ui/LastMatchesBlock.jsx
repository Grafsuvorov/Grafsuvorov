...
export default function LastMatchesBlock({
    ...
    <div
      className={[
        isTable
          ...
          : isSoft
          ? "space-y-3"
          : "rounded-3xl px-4 py-4 space-y-4 border border-violet-500/20 bg-slate-950/90 shadow-[0_18px_55px_rgba(0,0,0,0.5)]",
      ].join(" ")}
    >
      {/* HEADER */}
      <div>
        ...
        <div className="text-[11px] text-white/55 truncate">{title}</div>
      </div>

      {/* EMPTY */}
      {!list.length && (
        <div className="text-[11px] text-white/55">
          Нет данных по последним матчам.
        </div>
      )}
