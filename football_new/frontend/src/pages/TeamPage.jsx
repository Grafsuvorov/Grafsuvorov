...
export default function TeamPage() {
  ...
  useEffect(()=> {
    ...
    (async ()=>{
      ...
      const [o, r] = await Promise.all([
        fetch(`/api/team/overview?team_id=${id}&${base}`).then(x=>x.json()),
        fetch(`/api/team/rolling?team_id=${id}&${base}&window=10`).then(x=>x.json())
      ]);
  ...
  return (
    <div className="space-y-3">
      ...
      <Card className="border border-glass bg-surface-1/80 text-slate-100">
        <CardContent className="p-3">
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
            {["xg","xga","shots","possession","tempo"].map((k)=>(
              <div key={k} className="rounded-lg border border-glass bg-surface-2/70 p-3">
                <div className="text-[12px] text-slate-400">{k.toUpperCase()}</div>
