insert into  dm_calc.investment_payments_actual_group_02 (
...
purchase_document_expense_assignment as  (
	select
	ea.purchase_document_code ,
	ea.purchase_document_line_item_code ,
	ea.general_ledger_account_code ,
	row_number() over (partition by ea.purchase_document_code ,
	ea.purchase_document_line_item_code
order by
	ea.expense_assignment_code asc) as rn
from
	dds.purchase_document_expense_assignment ea
where
	ea.quantity <> 0
)
select
    'PAY_MM' as expense_or_payment_source_type_code,
	...
	apr.purchase_document_position_reference_line_item_code as payment_request_position_code,
	apr.purchase_document_code as purchase_document_code,
	apr.purchase_document_position_line_item_code as purchase_document_position_code,
	null as reservation_document_code,
	null as reservation_document_position_code,
	null as reservation_document_reference_code,
	 ea.general_ledger_account_code as cost_element_code,
	...
	aec.external_contract_number as external_contract_number,
	apr.wbs_element_code
	...
	case
		...
		case
			when apr.specification_currency_code is null 
		 then apr.document_currency_amount
			else apr.specification_currency_amount
	end as document_currency_amount,
	case 
			when apr.specification_currency_code is null then apr.document_currency_code
			else apr.specification_currency_code
	end as document_currency_code,
	...
	case
			when apr.specification_currency_code is null then apr.document_currency_vat_amount
			else apr.specification_currency_vat_amount
		--end
	end as vat_document_currency_amount,
	case
		when ad.debit_or_credit = 'S' then -1
		else 1
	end *
	case
			when apr.specification_currency_code is null then apr.document_currency_without_vat_amount
			else apr.specification_currency_amount - apr.specification_currency_vat_amount
	--	end
	end as exclude_vat_payment_document_currency_amount,
...
inner join dds.advance_payment_requirements_by_purchase_orders apr on
	apr.unit_balance_code = ad.unit_balance_code
	and apr.fiscal_year::numeric = ad.fiscal_year
	and apr.accounting_document_code = ad.accounting_document_code
	and apr.accounting_document_position_line_item_code::numeric = ad.position_line_item
	and apr.wbs_element_code <> '00000000'
left join purchase_document_expense_assignment ea on
	ea.purchase_document_code = apr.purchase_document_code
	and ea.purchase_document_line_item_code = apr.purchase_document_position_line_item_code
	and ea.rn = 1
left join dm_calc.accounting_external_contracts aec 
on
	aec.contract_code = apr.purchase_document_code
...
inner join prps as mdd on
	mdd.wbs_element_code = apr.wbs_element_code
left join
...
where
	1 = 1
	and (ad.earmarked_document_code is  null )
