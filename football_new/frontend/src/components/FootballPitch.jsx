    ...
    >
      <div className={`relative flex items-center justify-center rounded-full ring-2 ${ring}`} style={{ width: size, height: size }}>
        {!!imgSrc && !err ? (
          ...
          <div className="w-full h-full rounded-full flex items-center justify-center text-xs font-semibold text-slate-800">
            {p.number ? `#${p.number}` : initials(p.player_name || p.name)}
          </div>
        )}
      </div>
      {(p.player_name || p.name) && (
        <div className="mt-1 w-max px-2 py-0.5 rounded-full bg-white/80 backdrop-blur text-[10px] leading-none text-slate-800 shadow">
          {p.player_name || p.name}
        </div>
      )}
    </div>
