...
def resolve_repo_root(args: argparse.Namespace) -> str:
    ...
    if os.path.isdir(os.path.join(cwd, "meta_info")):
        return cwd

    script_root = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
    if os.path.isdir(os.path.join(script_root, "meta_info")):
        return script_root

    return cwd


def build_default_out_dir(args: argparse.Namespace) -> str:
    return os.path.join(
        ...
        "etl_loads_entity",
        args.entity_name,
        args.target_schema.lower(),
        args.target_table.lower(),
    )
