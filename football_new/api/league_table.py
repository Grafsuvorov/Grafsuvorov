...
def league_table(
    ...
    with engine.connect() as conn:
        ...
        # Dedupe teams if API returns duplicate rows for the same team.
        df = df.sort_values(["rank", "team_id"], ascending=[True, True])
        df["_dedupe_key"] = df["team_id"].where(df["team_id"].notna(), df["team"])
