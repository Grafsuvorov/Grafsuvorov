...
SELECT
  ...
  /* rule3 */
  CASE
    WHEN (p.umkumo + p.abkumo + p.zukumo) = 0 THEN 0
    ELSE TRUNC(
           CAST(ROUND(
  ...
  /* rule2_prev_date — старая логика v1: с COALESCE(c_prev.salk3, 0) */
  CASE
    WHEN (p.abkumo + p.umkumo + p.zukumo) <> 0 THEN
      TRUNC(
        CAST(
