...
from config import (
    ...
    VAL_DAYS,
    GAP_DAYS,
    TOTALS_USE_SELECTED_UNDERSTAT,
    TOTALS_USE_SELECTED_TEAM_POTENTIAL,
...
def select_totals_feature_cols(df: pd.DataFrame) -> List[str]:
    from train_outcomes import build_safe_feature_list

    feature_cols = build_safe_feature_list(df)
    if not TOTALS_USE_SELECTED_UNDERSTAT:
        return feature_cols

    selected = set(TOTALS_UNDERSTAT_SELECTED_FEATURES)
    filtered = []
    for c in feature_cols:
        if c.startswith(UNDERSTAT_PREFIXES):
            if c in selected:
                filtered.append(c)
        elif c.startswith(TEAM_POTENTIAL_PREFIX):
            if (not TOTALS_USE_SELECTED_TEAM_POTENTIAL) or (c in set(TOTALS_TEAM_POTENTIAL_SELECTED_FEATURES)):
                filtered.append(c)
        else:
            filtered.append(c)
...
def _train_totals_single(
    ...
    if TOTALS_USE_SELECTED_UNDERSTAT and "season" in train.columns:
        train = train[train["season"].astype(int) >= int(TOTALS_UNDERSTAT_MIN_SEASON)].copy().reset_index(drop=True)
    if train.empty:
    ...
    feature_cols = feature_cols_override or select_totals_feature_cols(train)
    feature_priors = (
        train[feature_cols]
    ...
    if have_mkt:
        ...
        best = (0.0, log_loss(y_cal, p_cal_mix, labels=[0, 1]))
        for a in np.linspace(0.0, 0.6, 7):
            mix_cal = _safe_blend_local(p_cal_mix[: len(pm_cal)], pm_cal, a)
