...
OUT_PATH = Path("tmp/combined_contour_v1_last_month.txt")
...
def summarize(frame: pd.DataFrame, by: list[str]) -> pd.DataFrame:
    out = (
        ...
        .agg(
            bets=("market", "size"),
            wins=("won", "sum"),
            losses=("won", lambda x: int((1 - x).sum())),
            profit=("profit", "sum"),
        )
        .reset_index()
    )
...
def main():
    ...
    with engine.connect() as conn:
        base = pd.read_sql(q, conn, params={"date_from": DATE_FROM, "date_to": DATE_TO})
    ...
    weekly_by_league_market = summarize(df, ["week_start", "league", "market"])
    weekly_totals = summarize(df, ["week_start"])
    overall_by_league_market = summarize(df, ["league", "market"])

    parts = [
        "WINDOW",
        f"{DATE_FROM} -> {DATE_TO}",
        "",
        "WEEKLY_BY_LEAGUE_MARKET",
        weekly_by_league_market.to_string(index=False),
        "",
        "WEEKLY_TOTALS",
        weekly_totals.to_string(index=False),
        "",
        "OVERALL_BY_LEAGUE_MARKET",
        overall_by_league_market.to_string(index=False),
        "",
    ]
    report = "\n".join(parts)
    OUT_PATH.write_text(report, encoding="utf-8")
