...
def _saved_recommendation(rec_raw: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    best_type = str(rec_raw.get("best_bet_type") or "").strip().upper()
    best_outcome = str(rec_raw.get("best_bet_outcome") or "").strip()
    bet_rating = str(rec_raw.get("bet_rating") or "").strip().lower()
    best_ev = _safe(rec_raw.get("best_bet_ev"))
    best_odds = _safe(rec_raw.get("best_bet_odds"))

    if not best_type or best_type == "NONE" or not best_outcome:
        return None

    tier = "A" if bet_rating == "strong" else "B" if bet_rating in {"medium", "weak"} else None

    if best_type == "1X2":
        outcome_map = {"Home": "home", "Draw": "draw", "Away": "away"}
        outcome = outcome_map.get(best_outcome)
        prob_map = {
            "home": rec_raw.get("p_home"),
        ...
        if outcome:
            return {
                "market": "1X2",
                "outcome": outcome,
                "p": prob_map.get(outcome),
                "odds": best_odds,
                "ev": best_ev,
                "label": _label_for("1X2", outcome),
                "tier": tier,
                "saved": True,
            }

    if best_type == "TOTAL":
        outcome_map = {"Over2.5": "over", "Under2.5": "under"}
        outcome = outcome_map.get(best_outcome)
        prob_map = {
            "over": rec_raw.get("p_over25"),
            "under": rec_raw.get("p_under25"),
        }
        if outcome:
            return {
                "market": "OU25",
...
def guard_dual(rec_raw: Dict[str, Any], league_name: str) -> Dict[str, Any]:
    ...
    saved = _saved_recommendation(rec_raw)
    selected_market = None
    selected_outcome = None
    selected_tier = None
    if saved:
        selected_market = saved.get("market")
        selected_outcome = saved.get("outcome")
        selected_tier = saved.get("tier")
        if saved["market"] == "1X2":
            rec_outcome = saved
        elif saved["market"] == "OU25":
            rec_total = saved
