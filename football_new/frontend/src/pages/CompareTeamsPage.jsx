...
function TeamBadge({ id, name, align = "left" }) {
  const badge = (
    <TeamLogoLink teamId={id} className="h-12 w-12" title={name}>
      <span className="h-12 w-12 rounded-xl grid place-items-center overflow-hidden bg-surface-2 border border-glass">
  ...
  return (
    <div className={`flex items-center gap-2 min-w-0 ${align === "right" ? "justify-end" : ""}`}>
      {align === "left" && badge}
      <span className={`truncate font-semibold text-slate-100 ${align === "right" ? "text-right" : ""}`}>{name || "—"}</span>
      {align === "right" && badge}
    </div>
  );
}
...
function CompareRow({ label, a, b, fmt = (v) => (v == null ? "—" : Number(v).toFixed(2)) }) {
  ...
  return (
    <div className="grid grid-cols-12 items-center gap-2">
      <div className="col-span-3 text-xs text-slate-400">{label}</div>
      <div className="col-span-4">
        <div className="text-[11px] text-slate-400">{fmt(a)}</div>
        <div className="h-1.5 rounded-full bg-white/10">
          <div className="h-1.5 rounded-full bg-violet-500/80" style={{ width: `${wa}%` }} />
        </div>
      </div>
      <div className="col-span-1 text-center text-[11px] text-slate-500">—</div>
      <div className="col-span-4">
        <div className="text-[11px] text-slate-400 text-right">{fmt(b)}</div>
        <div className="h-1.5 rounded-full bg-white/10 relative">
          <div className="h-1.5 rounded-full bg-white/25 absolute right-0" style={{ width: `${wb}%` }} />
        </div>
      </div>
    </div>
  );
...
function PickTeamsModal({ league, season, teams, aId, bId, onPick, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[100]">
      ...
      <div className="absolute left-1/2 top-16 -translate-x-1/2 w-[min(920px,96vw)] rounded-2xl border border-glass bg-surface-1/95 shadow-2xl">
        <button
          onClick={onClose}
          className="absolute -right-3 -top-3 h-9 w-9 rounded-full bg-surface-2 border border-glass shadow grid place-items-center hover:bg-surface-1/80 text-slate-200"
          aria-label="Закрыть"
          title="Закрыть"
        >×</button>

        <div className="px-4 py-3 border-b border-glass bg-surface-1/95 rounded-t-2xl">
          <div className="text-sm text-slate-200">Выберите две команды · {league} {season}</div>
          <div className="mt-2">
            <input
              value={q}
              onChange={(e)=>setQ(e.target.value)}
              placeholder="Найти команду…"
              className="w-full h-10 rounded-lg border border-glass bg-surface-2 px-3 text-sm text-slate-100 placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-primary/40"
            />
          </div>
        </div>
          ...
          {filtered.map((t) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-3 rounded-lg border p-2 text-left hover:shadow-sm transition ${selected ? "border-primary/70" : "border-glass"} bg-surface-2/80`}
                title={t.team}
              >
                <span className="h-9 w-9 rounded-full grid place-items-center overflow-hidden bg-surface-2 border border-glass">
                  <SafeImg src={teamLogo(t.team_id)} alt={t.team} className="h-6 w-6 object-contain" />
                </span>
                <span className="text-sm font-medium truncate text-slate-100">{t.team}</span>
              </button>
            );
          })}
          {!filtered.length && <div className="col-span-full text-sm text-slate-400">Ничего не найдено.</div>}
        </div>
      </div>
    </div>
  );
...
export default function CompareTeamsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const r = await fetch(`/api/league/teams?${qs.toString()}`);
  ...
  useEffect(() => {
    ...
    async function load(id, setOverview) {
      ...
      try {
        ...
        const o = await fetch(`/api/team/overview?team_id=${id}&${base}`).then((x) => x.json());
  ...
  return (
    <div className="space-y-3 text-slate-200">
      ...
      <button
        onClick={goTable}
        className="fixed right-4 top-4 z-40 h-9 w-9 rounded-full bg-surface-2 border border-glass shadow hover:bg-surface-1/70 text-slate-200"
        ...
        <CardContent className="p-3">
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              <span className="h-9 w-9 rounded-lg bg-surface-2 border border-glass grid place-items-center overflow-hidden">
                <SafeImg src={leagueLogo(league)} alt={league} className="h-7 w-7 object-contain" />
              </span>
              <div className="text-sm font-semibold text-slate-100">{league} {season}</div>
            </div>
            <button onClick={() => setOpenPick(true)} className="h-8 px-3 rounded-lg border border-glass text-sm bg-surface-2 hover:bg-surface-1/70">
              Выбрать команды
            </button>
          </div>
