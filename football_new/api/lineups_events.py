...
def get_lineups_events(
    ...
    try:
        ...
        logger.exception("lineups_events failed for fixture_id=%s", fixture_id)
        raise HTTPException(status_code=500, detail=f"lineups-events failed: {e}")
...
def get_match_events(
    ...
    """
    Совместимость: вернуть только события.
    Используется фронтом, если нет lineups-events.
    """
    data = get_lineups_events(fixture_id)
