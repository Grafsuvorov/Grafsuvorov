...
def _is_international_league(league: Optional[str]) -> bool:
    return str(league or "").strip() in INTERNATIONAL_LEAGUES
...
def team_overview(
    ...
    try:
        ...
        with engine.begin() as con:
            q = text("""
            -- 1) league_name: если передали league -> используем его, иначе берём последний по team+season
            WITH ln AS (
              SELECT COALESCE(
                :league,
            ...
            -- 5) league_id: параметр → schedule (по ln.league_name) → standings
            lid AS (
              SELECT COALESCE(
                :league_id,
                (SELECT sc.league_id
            ...
            SELECT
              ...
              COALESCE((SELECT matches_played FROM res_agg), (SELECT matches_played FROM ts)) AS matches_played,
              COALESCE((SELECT wins           FROM res_agg), (SELECT wins           FROM ts)) AS wins,
              COALESCE((SELECT draws          FROM res_agg), (SELECT draws          FROM ts)) AS draws,
              COALESCE((SELECT losses         FROM res_agg), (SELECT losses         FROM ts)) AS losses,
              COALESCE((SELECT goals_for      FROM res_agg), (SELECT goals_for      FROM ts)) AS goals_for,
              COALESCE((SELECT goals_against  FROM res_agg), (SELECT goals_against  FROM ts)) AS goals_against,
        ...
        # totals
        xg_total  = pick((r.get("xg_avg_ms")  * mp if (r.get("xg_avg_ms")  is not None and mp) else None), r.get("xg_total_ts"))
        xga_total = pick((r.get("xga_avg_ms") * mp if (r.get("xga_avg_ms") is not None and mp) else None), r.get("xga_total_ts"))
        ...
        shots_avg         = pick(r.get("shots_avg_ms"), r.get("shots_avg_ts"))
        possession_avg    = pick(r.get("poss_avg_ms"),  r.get("poss_avg_ts"))
        tempo_avg         = pick(r.get("tempo_avg_ms"), r.get("tempo_avg_ts"))
...
def team_results(
    team_id: int = Query(..., description="ID команды"),
    league: Optional[str] = Query(None, description="Название лиги"),
    season: Optional[str] = Query(None, description="Год сезона, напр. 2025"),
    ...
    """
    try:
        is_intl = _is_international_league(league)
        q = """
        WITH base AS (
          SELECT
            s.fixture_id,
          ...
          FROM football.api_football_schedule s
          WHERE (:is_intl = TRUE OR :league IS NULL OR s.league_name = :league)
            AND (:is_intl = TRUE OR :season IS NULL OR s.season::text = :season)
        ...
        stats AS (
          SELECT
            ...
            h.total_shots     AS home_shots,
            a.total_shots     AS away_shots,
            h.shots_on_goal   AS home_sot,
            a.shots_on_goal   AS away_sot,
            NULLIF(regexp_replace(COALESCE(h.possession::text,''), '[^0-9\\.]', '', 'g'),'')::numeric AS home_poss,
            NULLIF(regexp_replace(COALESCE(a.possession::text,''), '[^0-9\\.]', '', 'g'),'')::numeric AS away_poss
          FROM base b
          LEFT JOIN football.api_football_match_stats h
        ...
        SELECT
          ...
          CASE WHEN b.home_team_id = :team_id THEN s.home_xg ELSE s.away_xg END AS xg,
          CASE WHEN b.home_team_id = :team_id THEN s.away_xg ELSE s.home_xg END AS xg_opp,
          CASE WHEN b.home_team_id = :team_id THEN s.home_shots ELSE s.away_shots END AS shots,
          CASE WHEN b.home_team_id = :team_id THEN s.away_shots ELSE s.home_shots END AS shots_opp,
          CASE WHEN b.home_team_id = :team_id THEN s.home_sot ELSE s.away_sot END AS shots_on_goal,
          CASE WHEN b.home_team_id = :team_id THEN s.away_sot ELSE s.home_sot END AS shots_on_goal_opp,
          CASE WHEN b.home_team_id = :team_id THEN s.home_poss ELSE s.away_poss END AS possession,
          CASE WHEN b.home_team_id = :team_id THEN s.away_poss ELSE s.home_poss END AS possession_opp

        FROM base b
        LEFT JOIN stats s USING (fixture_id)
...
def team_schedule(
    ...
    Для сборных не ограничивается одной лигой и сезоном.
    """
    try:
        is_intl = _is_international_league(league)
        q = """
        SELECT
          s.fixture_id,
          to_char(s.date, 'YYYY-MM-DD"T"HH24:MI:SS') AS datetime,
        ...
        WHERE (:is_intl = TRUE OR :league IS NULL OR s.league_name = :league)
          ...
          AND (s.home_team_id = :team_id OR s.away_team_id = :team_id)
          AND (
                s.date >= now() - interval '1 day'
             OR COALESCE(s.status_short, '') IN ('1H', '2H', 'HT', 'ET', 'P', 'BT', 'LIVE')
          )
          AND NOT (
                COALESCE(s.status,'') IN ('Match Finished','FT','FT_PEN','AET','PEN','AWARDED','Awarded','Abandoned','Canceled')
                AND s.date < now() - interval '1 day'
          )
        ORDER BY s.date ASC, s.fixture_id ASC
        LIMIT :limit
        """
        with engine.begin() as con:
            rows = [
        ...
        logger.exception("team_schedule failed")
        raise HTTPException(status_code=500, detail=str(e))
