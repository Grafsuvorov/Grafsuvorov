...
BASE_CATEGORICAL_COLS = ["league_id", "season", "round", "home_team_id", "away_team_id"]
...
def fit_catboost_base(train_df: pd.DataFrame, cal_df: pd.DataFrame) -> CatBoostClassifier:
    ...
    model.fit(
        x_train,
        y_train,
        cat_features=[x_train.columns.get_loc(c) for c in BASE_CATEGORICAL_COLS],
...
def predict_catboost_base(model: CatBoostClassifier, df: pd.DataFrame) -> np.ndarray:
    x, _ = _prepare_xy(df)
